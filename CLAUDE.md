...
| --- | --- |
| `*FullereneUnlabeledEnum*` | enumeration takes hours from n>=20 (all cases excluded) |
| `*CubicPlanarLabeledEnum*case6` | n=10 (5,826,240 graphs) takes ~280 s |
| `*/CircularArcLabeledEnumTest.*case6` | n=6 takes ~250 s alone vs ~5 s for all the rest; the leading `/` keeps `ProperCircularArcLabeledEnumTest` included |
...
simple_quadrangulation, chain, cochain, threshold, maximal_outer_planar, apollonian); where a labeled enumerator exists
...
- CLI `src/<type>_unlabeled_enum_main.cpp` → `bin/<type>_unlabeled_enum`, taking
  `--connected` when the enumerator supports `connected_only`.
...
- When the class also has a labeled enumerator, tests must include a cross-check
  that canonicalizing the labeled enumerator's output
