...
上記フィルタ下での実測値 (2026-08-25, NO 証明書の導入後): 1123 テスト / 182 テストスイート、全て PASS。gtest 実行時間はアイドル時 約 18 秒 (以前の記載 13 秒との差は測定機の違いによるもの。証明書導入の直前後で同一機・同一条件の実測が 17.8 秒 → 18.5 秒なので、NO パスの抽出自体の増分は 1 秒未満)。`make test-quick` は 1175 テスト / 約 185 秒 (内訳は n=7 全数検証が中心で、split 探索 約 70 秒、DMP 平面埋め込み 約 30 秒、SPQR 約 26 秒)。ヘッダ変更後の初回は `make test` にフルリビルドの +50 秒程度が加わる。かつて全体の 8 割以上を占めていた `CircleEnumTest/case6` (n=6, 32636 グラフ, 単独 20 秒) は、circle 認識の Naji 化により約 0.2 秒に短縮された。
