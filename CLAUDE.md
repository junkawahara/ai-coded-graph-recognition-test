...
include/       ヘッダオンリーライブラリ (全アルゴリズム)
  ...
  ...            (その他 150 ヘッダ; include/ 全体で 165 ファイル)
...
上記フィルタ下での実測値 (2026-08-25): 1051 テスト / 172 テストスイート、全て PASS。gtest 実行時間はアイドル時 約 13 秒 (`make test-quick` は 1099 テスト)。ヘッダ変更後の初回は `make test` にフルリビルドの +50 秒程度が加わる。かつて全体の 8 割以上を占めていた `CircleEnumTest/case6` (n=6, 32636 グラフ, 単独 20 秒) は、circle 認識の Naji 化により約 0.2 秒に短縮された。
