...
enumerate_circle_graphs_reverse_search(int n,
    CircleEnumAlgorithm algo = CircleEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    CircleEnumerationResult result;
    if (n < 0) return result;
    detail::CircleEnumState root(n);
    detail::circle_enum_dfs(root, &result.graphs);
    return result;
}
