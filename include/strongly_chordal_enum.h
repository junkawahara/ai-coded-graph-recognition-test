...
template <typename Callback>
inline void enumerate_strongly_chordal_graphs_kiyomi_cb(int n,
                                                        Callback& cb) {
...
template <typename Callback>
inline void strongly_chordal_legacy_dfs(const ChordalEnumState& state,
                                        Callback& cb) {
 ...
 */
inline StronglyChordalEnumerationResult
enumerate_strongly_chordal_graphs_reverse_search(
    int n,
    StronglyChordalEnumAlgorithm algo =
        StronglyChordalEnumAlgorithm::KIYOMI_EDGE_ADDITION) {
    StronglyChordalEnumerationResult result;
    if (n < 0) return result;

    detail::AppendToVector cb;
    cb.out = &result.graphs;
    if (algo == StronglyChordalEnumAlgorithm::LEGACY_CHORDAL_FILTER) {
        detail::ChordalEnumState root(n);
        detail::strongly_chordal_legacy_dfs(root, cb);
    } else {
        detail::enumerate_strongly_chordal_graphs_kiyomi_cb(n, cb);
    }
    return result;
}
 ...
 */
template <typename Callback>
inline void enumerate_strongly_chordal_graphs_reverse_search_cb(
    int n,
    Callback&& cb,
    StronglyChordalEnumAlgorithm algo =
        StronglyChordalEnumAlgorithm::KIYOMI_EDGE_ADDITION) {
