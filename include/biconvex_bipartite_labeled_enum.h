...
/**
 * @file biconvex_bipartite_labeled_enum.h
 * @brief Biconvex bipartite graph enumeration (reverse search)
 *
 * Enumerates all labeled biconvex bipartite graphs on vertex set {1, ..., n}
 * using reverse search.
 *
 * parent(G) = remove the vertex with the largest label from G
 * (biconvex bipartite is a hereditary class, so this is always valid)
 *
 * Vertices are added in order 1, 2, ..., n, and at each step all
 * subsets of possible neighborhoods are enumerated and filtered by biconvex bipartite test.
 */
...
/**
 * @brief Algorithm selection for biconvex bipartite enumeration
 */
enum class BiconvexBipartiteLabeledEnumAlgorithm {
    REVERSE_SEARCH /**< reverse search */
};

/**
 * @brief Result of biconvex bipartite enumeration
 */
struct BiconvexBipartiteLabeledEnumerationResult {
    std::vector<EnumeratedGraph> graphs; /**< array of enumerated biconvex bipartite graphs */
};
...
/**
 * @brief DFS for biconvex bipartite reverse search
 *
 * Adds vertex alive_count+1 and tries all subsets of {1,...,alive_count}
 * as its neighborhood. Prunes children that are not biconvex bipartite.
 */
...
/**
 * @brief Enumerates all labeled biconvex bipartite graphs on vertex set {1, ..., n}
 * @param n Number of vertices
 * @param algo Algorithm selection (currently only REVERSE_SEARCH)
 * @return BiconvexBipartiteLabeledEnumerationResult
 *
 * Uses reverse search. parent(G) is obtained by removing the vertex
 * with the largest label from G. Biconvex bipartite is a hereditary
 * class, so the property is preserved under any vertex removal.
 */
inline BiconvexBipartiteLabeledEnumerationResult
enumerate_biconvex_bipartite_labeled_graphs_reverse_search(int n,
    BiconvexBipartiteLabeledEnumAlgorithm algo =
        BiconvexBipartiteLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    BiconvexBipartiteLabeledEnumerationResult result;
