 ...
 * @brief Weakly chordal graph enumeration (reverse search)
 *
 * Enumerates all labeled weakly chordal graphs on vertex set {1, ..., n}
 * using reverse search.
 *
 * parent(G) = removal of the vertex with the largest label from G
 * (always valid since weakly chordal is a hereditary class)
 *
 * Vertices are added in the order 1, 2, ..., n, and at each step all
 * possible neighborhood subsets are enumerated and filtered by weakly chordal recognition.
 */
...
struct WeaklyChordalEnumerationResult {
    std::vector<EnumeratedGraph> graphs; /**< Array of enumerated weakly chordal graphs */
};
...
/**
 * @brief DFS for weakly chordal reverse search
 *
 * Adds vertex alive_count+1 and tries all subsets of {1,...,alive_count}
 * as its neighborhood. Prunes children that are not weakly chordal.
 */
...
/**
 * @brief Enumerates all labeled weakly chordal graphs on vertex set {1, ..., n}
 * @param n Number of vertices
 * @param algo Algorithm selection (currently only REVERSE_SEARCH)
 * @return WeaklyChordalEnumerationResult
 *
 * Uses reverse search. parent(G) is obtained by removing the vertex
 * with the largest label from G. Since weakly chordal is a hereditary class,
