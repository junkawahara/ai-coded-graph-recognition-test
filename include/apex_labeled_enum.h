#ifndef GRAPH_RECOGNITION_APEX_ENUM_H
#define GRAPH_RECOGNITION_APEX_ENUM_H

/**
 * @file apex_labeled_enum.h
 ...
 */
enum class ApexLabeledEnumAlgorithm {
 ...
 */
struct ApexLabeledEnumerationResult {
...
struct ApexLabeledEnumState {
    int total_n;
    int alive_count;  /**< active vertices are {1, ..., alive_count} */
    std::vector<std::vector<char>> adj;

    explicit ApexLabeledEnumState(int n)
 ...
 */
inline void apex_labeled_enum_dfs(ApexLabeledEnumState& state,
    ...
    for (unsigned long long mask = 0; mask < limit; ++mask) {
        ...
        Graph g(x, edges);
        ApexResult res = check_apex(g);

        if (res.is_apex) {
            apex_labeled_enum_dfs(state, out);
        }
 ...
 * @return ApexLabeledEnumerationResult
 *
 ...
 */
inline ApexLabeledEnumerationResult
enumerate_apex_labeled_graphs_reverse_search(int n,
    ApexLabeledEnumAlgorithm algo =
        ApexLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    ApexLabeledEnumerationResult result;
    if (n < 0) return result;
    detail::ApexLabeledEnumState root(n);
    detail::apex_labeled_enum_dfs(root, &result.graphs);
    return result;
}
