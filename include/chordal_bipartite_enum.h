 ...
 * (chordal bipartite は遺伝的クラスのため常に有効)
 *
 * 頂点を 1, 2, ..., n の順に追加し、各ステップで可能な近傍の
 * すべての部分集合を列挙して chordal bipartite 判定でフィルタする。
 */
...
/**
 * @brief Chordal Bipartite 列挙アルゴリズムの選択
 */
enum class ChordalBipartiteEnumAlgorithm {
    REVERSE_SEARCH /**< 逆探索 */
};

/**
 * @brief Chordal Bipartite 列挙の結果
 */
struct ChordalBipartiteEnumerationResult {
    std::vector<EnumeratedGraph> graphs; /**< 列挙された弦二部グラフの配列 */
};
...
/**
 * @brief Chordal Bipartite 逆探索の DFS
 *
 * 頂点 alive_count+1 を追加し、{1,...,alive_count} の部分集合を
 * 近傍として試す。chordal bipartite でない子を枝刈りする。
 */
...
/**
 * @brief 頂点集合 {1, ..., n} 上のラベル付き Chordal Bipartite グラフを全列挙する
 * @param n 頂点数
 * @param algo アルゴリズム選択 (現在は REVERSE_SEARCH のみ)
 * @return ChordalBipartiteEnumerationResult
 *
 * 逆探索 (reverse search) を使用。parent(G) は G から最大ラベルの
 * 頂点を除去して得られる。chordal bipartite は遺伝的クラスの
 * ため、任意の頂点の除去で性質が保存される。
 */
inline ChordalBipartiteEnumerationResult
enumerate_chordal_bipartite_graphs_reverse_search(int n,
    ChordalBipartiteEnumAlgorithm algo =
        ChordalBipartiteEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    ChordalBipartiteEnumerationResult result;
