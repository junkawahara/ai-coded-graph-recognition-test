...
/**
 * @file comparability_labeled_enum.h
 ...
 */
enum class ComparabilityLabeledEnumAlgorithm {
 ...
 */
struct ComparabilityLabeledEnumerationResult {
...
struct ComparabilityLabeledEnumState {
    int total_n;
    int alive_count;  /**< active vertices are {1, ..., alive_count} */
    std::vector<std::vector<char>> adj;

    explicit ComparabilityLabeledEnumState(int n)
 ...
 */
inline void comparability_labeled_enum_dfs(ComparabilityLabeledEnumState& state,
    ...
    for (unsigned long long mask = 0; mask < limit; ++mask) {
        ...
        if (res.is_comparability) {
            comparability_labeled_enum_dfs(state, out);
        }
 ...
 * @return ComparabilityLabeledEnumerationResult
 *
 ...
 */
inline ComparabilityLabeledEnumerationResult
enumerate_comparability_labeled_graphs_reverse_search(int n,
    ComparabilityLabeledEnumAlgorithm algo =
        ComparabilityLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    ComparabilityLabeledEnumerationResult result;
    if (n < 0) return result;
    detail::ComparabilityLabeledEnumState root(n);
    detail::comparability_labeled_enum_dfs(root, &result.graphs);
    return result;
}
