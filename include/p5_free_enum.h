...
enumerate_p5_free_graphs_reverse_search(int n,
    P5FreeEnumAlgorithm algo =
        P5FreeEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    P5FreeEnumerationResult result;
    if (n < 0) return result;
    detail::P5FreeEnumState root(n);
    detail::p5_free_enum_dfs(root, &result.graphs);
    return result;
}
