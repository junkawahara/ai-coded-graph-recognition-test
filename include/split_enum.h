...
template <typename Callback>
inline void enumerate_split_graphs_canonical_ks_cb(int n, Callback& cb) {
    std::vector<int> clique;
    std::vector<int> stable;
...
template <typename Callback>
inline void split_legacy_reverse_search_dfs(const ChordalEnumState& state,
                                            Callback& cb) {
 ...
 */
inline SplitEnumerationResult enumerate_split_graphs_reverse_search(
    int n,
    SplitEnumAlgorithm algo =
        SplitEnumAlgorithm::KS_PARTITION_CANONICAL) {
    SplitEnumerationResult result;
    if (n < 0) return result;

    detail::AppendToVector cb;
    cb.out = &result.graphs;
    if (algo == SplitEnumAlgorithm::LEGACY_CHORDAL_FILTER) {
        detail::ChordalEnumState root(n);
        detail::split_legacy_reverse_search_dfs(root, cb);
    } else {
        detail::enumerate_split_graphs_canonical_ks_cb(n, cb);
    }
    return result;
}
 ...
 */
template <typename Callback>
inline void enumerate_split_graphs_reverse_search_cb(
    int n,
    Callback&& cb,
    SplitEnumAlgorithm algo =
        SplitEnumAlgorithm::KS_PARTITION_CANONICAL) {
