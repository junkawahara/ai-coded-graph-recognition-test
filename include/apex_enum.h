...
enumerate_apex_graphs_reverse_search(int n,
    ApexEnumAlgorithm algo =
        ApexEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    ApexEnumerationResult result;
    if (n < 0) return result;
    detail::ApexEnumState root(n);
    detail::apex_enum_dfs(root, &result.graphs);
    return result;
}
