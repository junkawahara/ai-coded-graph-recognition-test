...
/**
 * @file circular_arc_labeled_enum.h
 ...
 */
enum class CircularArcLabeledEnumAlgorithm {
 ...
 */
struct CircularArcLabeledEnumerationResult {
...
struct CircularArcLabeledEnumState {
    int total_n;
    int alive_count; /**< active vertices are {1, ..., alive_count} */
    std::vector<std::vector<char>> adj;

    explicit CircularArcLabeledEnumState(int n)
 ...
 */
inline void circular_arc_labeled_enum_dfs(CircularArcLabeledEnumState& state,
    ...
    for (unsigned long long mask = 0; mask < limit; ++mask) {
        ...
        if (res.is_circular_arc) {
            circular_arc_labeled_enum_dfs(state, out);
        }
 ...
 * @return CircularArcLabeledEnumerationResult
 *
 ...
 */
inline CircularArcLabeledEnumerationResult
enumerate_circular_arc_labeled_graphs_reverse_search(int n,
    CircularArcLabeledEnumAlgorithm algo =
        CircularArcLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    CircularArcLabeledEnumerationResult result;
    if (n < 0) return result;
    detail::CircularArcLabeledEnumState root(n);
    detail::circular_arc_labeled_enum_dfs(root, &result.graphs);
    return result;
}
