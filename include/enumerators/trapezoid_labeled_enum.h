 ...
 */
enum class TrapezoidLabeledEnumAlgorithm {
 ...
 */
struct TrapezoidLabeledEnumerationResult {
...
struct TrapezoidLabeledEnumState {
    int total_n;
    int alive_count; /**< Active vertices are {1, ..., alive_count} */
    std::vector<std::vector<char>> adj;

    explicit TrapezoidLabeledEnumState(int n)
 ...
 */
inline void trapezoid_labeled_enum_dfs(TrapezoidLabeledEnumState& state,
    ...
    for (unsigned long long mask = 0; mask < limit; ++mask) {
        ...
        if (res.is_trapezoid) {
            trapezoid_labeled_enum_dfs(state, out);
        }
 ...
 * @return TrapezoidLabeledEnumerationResult
 *
 ...
 */
inline TrapezoidLabeledEnumerationResult
enumerate_trapezoid_labeled_graphs_reverse_search(int n,
    TrapezoidLabeledEnumAlgorithm algo =
        TrapezoidLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    TrapezoidLabeledEnumerationResult result;
    if (n < 0) return result;
    detail::TrapezoidLabeledEnumState root(n);
    detail::trapezoid_labeled_enum_dfs(root, &result.graphs);
    return result;
}
