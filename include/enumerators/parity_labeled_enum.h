 ...
 */
enum class ParityLabeledEnumAlgorithm {
 ...
 */
struct ParityLabeledEnumerationResult {
...
struct ParityLabeledEnumState {
    int total_n;
    int alive_count; /**< Alive vertices are {1, ..., alive_count} */
    std::vector<std::vector<char>> adj;

    explicit ParityLabeledEnumState(int n)
 ...
 */
inline void parity_labeled_enum_dfs(ParityLabeledEnumState& state,
    ...
    for (unsigned long long mask = 0; mask < limit; ++mask) {
        ...
        if (res.is_parity) {
            parity_labeled_enum_dfs(state, out);
        }
 ...
 * @return ParityLabeledEnumerationResult
 *
 ...
 */
inline ParityLabeledEnumerationResult
enumerate_parity_labeled_graphs_reverse_search(int n,
    ParityLabeledEnumAlgorithm algo =
        ParityLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    ParityLabeledEnumerationResult result;
    if (n < 0) return result;
    detail::ParityLabeledEnumState root(n);
    detail::parity_labeled_enum_dfs(root, &result.graphs);
    return result;
}
