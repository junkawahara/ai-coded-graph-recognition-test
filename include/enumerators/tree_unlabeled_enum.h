 ...
 */
enum class TreeUnlabeledEnumAlgorithm {
 ...
 */
struct TreeUnlabeledEnumeratedGraph {
 ...
 */
struct TreeUnlabeledEnumerationResult {
    std::vector<TreeUnlabeledEnumeratedGraph> graphs;      /**< Array of enumerated trees */
};
 ...
 */
inline TreeUnlabeledEnumeratedGraph level_seq_to_tree_graph(
    const std::vector<int>& L, int n) {

    TreeUnlabeledEnumeratedGraph g;
    g.n = n;
    if (n <= 1) return g;
 ...
 */
inline TreeUnlabeledEnumeratedGraph join_rooted_trees(
 ...
 */
inline void tree_unlabeled_enum_combine_subtrees(
    ...
    for (int i = start; i < (int)trees.size(); ++i) {
        ...
        tree_unlabeled_enum_combine_subtrees(parts, idx + 1, i, chosen, cache, results);
        chosen.pop_back();
    }
}
 ...
 */
inline void tree_unlabeled_enum_partition_dfs(
    ...
    if (remaining == 0) {
        std::vector<std::vector<int> > chosen;
        tree_unlabeled_enum_combine_subtrees(parts, 0, 0, chosen, cache, results);
        return;
    }
    ...
    for (int s = upper; s >= 1; --s) {
        parts.push_back(s);
        tree_unlabeled_enum_partition_dfs(remaining - s, s, parts, cache, results);
        parts.pop_back();
    }
}
...
inline void compute_rooted_trees(
    ...
    std::vector<int> parts;
    tree_unlabeled_enum_partition_dfs(k - 1, k - 1, parts, cache, cache[k]);
 ...
 * @return TreeUnlabeledEnumerationResult
 *
 ...
 */
inline TreeUnlabeledEnumerationResult enumerate_tree_unlabeled_graphs(int n,
    TreeUnlabeledEnumAlgorithm algo = TreeUnlabeledEnumAlgorithm::LEVEL_SEQUENCE) {
    (void)algo;
    TreeUnlabeledEnumerationResult result;
