...
enum class CubicLabeledEnumAlgorithm {
    REVERSE_SEARCH
};

struct CubicLabeledEnumerationResult {
    std::vector<EnumeratedGraph> graphs;
};

namespace detail {

struct CubicLabeledEnumState {
    int total_n;
    int alive_count;
    std::vector<std::vector<char> > adj;
    std::vector<int> deg;

    explicit CubicLabeledEnumState(int n)
        : total_n(n), alive_count(0),
          adj(n + 1, std::vector<char>(n + 1, 0)),
          deg(n + 1, 0) {}
};

inline void cubic_labeled_enum_choose(CubicLabeledEnumState& state,
...
inline void cubic_labeled_enum_dfs(CubicLabeledEnumState& state,
                            std::vector<EnumeratedGraph>* out) {
    ...
    cubic_labeled_enum_choose(state, available, 0, 0,
...
inline void cubic_labeled_enum_choose(CubicLabeledEnumState& state,
    ...
    if (chosen_count >= min_size) {
        ...
        if (feasible) {
            cubic_labeled_enum_dfs(state, out);
        }
    ...
    for (std::size_t i = start; i < available.size(); ++i) {
        ...
        cubic_labeled_enum_choose(state, available, i + 1, chosen_count + 1,
 ...
 * @return CubicLabeledEnumerationResult
 */
inline CubicLabeledEnumerationResult
enumerate_cubic_labeled_graphs(int n,
    CubicLabeledEnumAlgorithm algo = CubicLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    CubicLabeledEnumerationResult result;
    if (n <= 0) return result;
    if (n % 2 != 0) return result;
    if (n < 4) return result;

    detail::CubicLabeledEnumState root(n);
    detail::cubic_labeled_enum_dfs(root, &result.graphs);
    return result;
}
