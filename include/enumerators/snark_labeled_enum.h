...
enum class SnarkLabeledEnumAlgorithm {
    REVERSE_SEARCH
};

struct SnarkLabeledEnumerationResult {
    std::vector<EnumeratedGraph> graphs;
};

namespace detail {

struct SnarkLabeledEnumState {
    int total_n;
    int alive_count;
    ...
    explicit SnarkLabeledEnumState(int n)
 ...
 */
inline bool snark_has_short_cycle(const SnarkLabeledEnumState& state,
                                   int x, int v) {
 ...
 */
inline bool snark_is_bridgeless(const SnarkLabeledEnumState& state) {
    int n = state.total_n;
 ...
 */
inline bool snark_is_cyc4ec(const SnarkLabeledEnumState& state) {
    int n = state.total_n;
    int full = (1 << n) - 1;
 ...
 */
inline bool snark_is_3_edge_colorable(const SnarkLabeledEnumState& state) {
    int n = state.total_n;
 ...
 */
inline int snark_count_components(const SnarkLabeledEnumState& state, int n_verts) {
 ...
 */
inline bool snark_is_connected(const SnarkLabeledEnumState& state) {
    int n = state.total_n;
...
/* forward declaration */
inline void snark_labeled_enum_choose(SnarkLabeledEnumState& state,
...
inline void snark_labeled_enum_dfs(SnarkLabeledEnumState& state,
                            std::vector<EnumeratedGraph>* out) {
    ...
    snark_labeled_enum_choose(state, available, 0, 0,
...
inline void snark_labeled_enum_choose(SnarkLabeledEnumState& state,
    ...
    if (chosen_count >= min_size) {
        ...
        if (feasible) {
            snark_labeled_enum_dfs(state, out);
        }
    ...
    for (std::size_t i = start; i < available.size(); ++i) {
        ...
        snark_labeled_enum_choose(state, available, i + 1, chosen_count + 1,
 ...
 * @return SnarkLabeledEnumerationResult
 */
inline SnarkLabeledEnumerationResult
enumerate_snark_labeled_graphs(int n,
    SnarkLabeledEnumAlgorithm algo = SnarkLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    SnarkLabeledEnumerationResult result;
    ...
    detail::SnarkLabeledEnumState root(n);
    detail::snark_labeled_enum_dfs(root, &result.graphs);
    return result;
}
