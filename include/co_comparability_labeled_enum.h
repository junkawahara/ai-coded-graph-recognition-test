...
/**
 * @file co_comparability_labeled_enum.h
 ...
 */
enum class CoComparabilityLabeledEnumAlgorithm {
 ...
 */
struct CoComparabilityLabeledEnumerationResult {
...
struct CoComparabilityLabeledEnumState {
    int total_n;
    int alive_count;  /**< active vertices are {1, ..., alive_count} */
    std::vector<std::vector<char>> adj;

    explicit CoComparabilityLabeledEnumState(int n)
 ...
 */
inline void co_comparability_labeled_enum_dfs(CoComparabilityLabeledEnumState& state,
    ...
    for (unsigned long long mask = 0; mask < limit; ++mask) {
        ...
        if (res.is_co_comparability) {
            co_comparability_labeled_enum_dfs(state, out);
        }
 ...
 * @return CoComparabilityLabeledEnumerationResult
 *
 ...
 */
inline CoComparabilityLabeledEnumerationResult
enumerate_co_comparability_labeled_graphs_reverse_search(int n,
    CoComparabilityLabeledEnumAlgorithm algo =
        CoComparabilityLabeledEnumAlgorithm::REVERSE_SEARCH) {
    (void)algo;
    CoComparabilityLabeledEnumerationResult result;
    if (n < 0) return result;
    detail::CoComparabilityLabeledEnumState root(n);
    detail::co_comparability_labeled_enum_dfs(root, &result.graphs);
    return result;
}
