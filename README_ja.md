...
| k-縮退グラフ (k-degenerate) | `degenerate.h` | O(n + m) | 認識: [Lick+ 70], [Matula+ 83]<br>列挙 (ラベルなし): [Bauer+ 10], [McKay 98] | すべての誘導部分グラフが次数 k 以下の頂点を持つ (k は入力パラメータ; 厳密な縮退数を常に報告し、NO 証明書は (k+1)-コア) |
...
| 比較可能性グラフ (Comparability) | `comparability.h` | O(nm) | 認識: [Gallai 67], [Golumbic 80]<br>列挙: [Avis+ 96]<br>列挙 (ラベルなし): [McKay 98] | 辺に推移的向き付けが可能 |
| 余比較可能性グラフ (Co-comparability) | `co_comparability.h` | O(n³) | 認識: [Gallai 67]<br>列挙: [Avis+ 96]<br>列挙 (ラベルなし): [McKay 98] | 補グラフが比較可能性グラフ |
