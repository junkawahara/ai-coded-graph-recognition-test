...
int main() {
    ...
    // Asteroidal triple check: O(n*(n+m) + n^3), uses components of G - N[v].
    ...
    for (int v = 1; v <= n; ++v) {
        ...
        int cid = 0;
        for (int s = 1; s <= n; ++s) {
            if (blocked[s] || comp[v][s] != -1) continue;
            cid++;
            comp[v][s] = cid;
            q.clear();
            q.push_back(s);
            for (size_t qi = 0; qi < q.size(); ++qi) {
                int x = q[qi];
                for (int y : g[x]) {
                    if (blocked[y] || comp[v][y] != -1) continue;
                    comp[v][y] = cid;
                    q.push_back(y);
                }
            }
    ...
    for (int a = 1; a <= n; ++a) {
        for (int b = a + 1; b <= n; ++b) {
            if (adjSet[a].find(b) != adjSet[a].end()) continue;
            for (int c = b + 1; c <= n; ++c) {
                if (adjSet[a].find(c) != adjSet[a].end()) continue;
                if (adjSet[b].find(c) != adjSet[b].end()) continue;
                int cab = comp[c][a];
                if (cab == -1 || cab != comp[c][b]) continue;
                int abc = comp[a][b];
                if (abc == -1 || abc != comp[a][c]) continue;
                int bca = comp[b][a];
                if (bca == -1 || bca != comp[b][c]) continue;
                cout << "NO\n";
                return 0;
            }
        }
    ...
    for (int i = 0; i < k; ++i) {
        ...
        while (!qq.empty()) {
            ...
            for (int u : tree[v]) {
                if (!seen[u]) { seen[u] = 1; qq.push(u); }
            }
        }
        if (compc.size() == 1) {
            pos[compc[0]] = offset + 1;
            offset += 1;
            continue;
        }
        int start = -1;
        ...
        if (idx != (int)compc.size()) {
            cout << "NO\n";
            return 0;
        }
        offset += (int)compc.size();
    }
