...
64 bit の隣接語を使うため、この列挙器が扱えるのは ``n <= 63`` であり、
範囲外では空の結果を返す。``bin/chordal_unlabeled_enum`` は範囲外の ``n`` を
拒否する。空の結果をそのまま件数として出力すると、その頂点数の弦グラフが
存在しないという誤った主張になるためである。
...
メモリ量も辺数に支配される。ホストの隣接行列も探索状態も探索頂点数の 2 乗に
比例するため、探索はホストの辺を持つ頂点 (高々 ``2m`` 個) だけで行い、出力を
元のラベルへ戻している。孤立頂点 ``10^4`` 個と辺 3 本のホストは部分グラフが
4 個だけの正当な入力であり、数百 MB を要してはならない。
