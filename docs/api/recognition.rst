...
.. list-table::
   ...
   * - ``CLASS_BASED`` **(デフォルト)**
     - Gamma クラス (辺の同値類) ごとに向き付けの整合性を検査する。未向き付け辺を1本ずつ選び、制約伝播で同一クラスの辺を一括向き付けする貪欲アルゴリズム。
...
.. list-table::
   ...
   * - ``HASH_TWINS`` **(デフォルト)**
     - XOR ハッシュによるインクリメンタルツイン検出。各頂点にランダム 64-bit weight を割当て、除去時に隣接頂点のハッシュを O(1) で更新。ハッシュ一致後に近傍リストの厳密比較で検証するため確定的に正しい。計算量: O(n + m) 期待。
