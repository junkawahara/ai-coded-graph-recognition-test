...
chordal graphs (McKay canonical construction path) and outputs the
complement of each. Complementation commutes with relabeling, so it is a
bijection on isomorphism classes, and the counts equal the chordal counts.
...
`OEIS A048193 <https://oeis.org/A048193>`_ (the chordal counts;
``1, 2, 4, 10, 27, 94, 393, 2119``) up to ``n = 8``, and the output was
