...
.. list-table::
   ...
   * - ``DEGREE_SEQUENCE_FAST`` **(default)**
     - Counting sort + two-pointer simulation, O(n) for the decision; the
       creation sequence it returns is replayed against the graph, so the
       call as a whole is O(n+m)
...
The materializing entry point holds all ``2^(n-1)`` graphs at once, which is
already 131,072 graphs at ``n = 18``; the callback API builds each binary
string independently and keeps one graph at a time. ``bin/threshold_unlabeled_enum``
uses it, prints the exact ``2^(n-1)`` count first, and rejects an ``n``
outside ``0..63`` (the bitmask range) rather than printing the empty result
as a count.
