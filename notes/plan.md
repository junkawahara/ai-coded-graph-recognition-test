...
- [x] **k-Degenerate** — labeled constructive enumeration via well-orderings with exact counting formula (Bauer–Krug–Wagner, ANALCO 2010); unlabeled via geng + degeneracy filter. Implemented as a non-isomorphic enumerator (`degenerate_unlabeled_enum.h`) via the geng-+-degeneracy-filter route on the generic canonical-augmentation machinery: the class is hereditary, so the new recognizer `degenerate.h` (`check_degenerate(g, k)` — O(n + m) bucket-queue minimum-degree peeling reporting the exact degeneracy on YES and NO alike, with the removal order as YES certificate and the (k+1)-core as NO certificate; k is an input since every graph is (n-1)-degenerate) prunes every candidate child. k = 1 reproduces the forests (A005195; trees A000055 connected) and k >= n-1 all graphs (A000088); the 2-degenerate counts (1, 2, 4, 10, 28, 105, 508, 3454, 31935 for n = 1..9) and 3-degenerate counts (1, 2, 4, 11, 33, 148, 950 for n = 1..7) are not in the OEIS and were verified against a brute-force degeneracy filter through n = 5. The output properly contains the partial k-tree classes from n = 5, k = 2 on (K4 with one edge subdivided is 2-degenerate with treewidth 3). C++/CLI only, matching the partial_ktree precedent (the extra k does not fit the Python factory signatures); the Bauer–Krug–Wagner labeled route is referenced in the header but not implemented
- [ ] **Toroidal (genus 1)** — enumerate all graphs, exclude planar ones, test torus embeddability (Mohar–Thomassen); OEIS A319114/A319115
- [x] **Cage / (k,g)-Graph** — orderly generation of k-regular graphs with girth constraints, as in GENREG (Meringer 1999). PDF: `references/meringer1999_genreg_cages.pdf`. Implemented as a non-isomorphic enumerator (`cage_unlabeled_enum.h`): the kregular enumerator (= the g <= 3 special case) with GENREG's girth constraint folded into the search — every new cycle passes through the added vertex, so girth >= g holds iff its neighbors are pairwise at distance >= g-2 (the generalization of the snark enumerator's g = 5 rule), enforced by per-level truncated-BFS far masks, plus a Moore-bound cutoff; the girth parameter is a lower bound as in GENREG, so the cages are the output at the smallest nonempty n. g = 3 reproduces the kregular classes (A051031), k = 2 the partitions of n into parts >= g, (3,5) yields exactly the Petersen graph at n = 10 (counts 1, 2, 9 at n = 10, 12, 14; A014372) and (3,6) exactly the Heawood graph at n = 14. New recognizer `cage.h` (`check_cage(g, k, girth)` — exact girth by truncated BFS, is_kg_graph = k-regular + girth >= g, and is_cage decided exactly by the enumerator's early-exit existence search `cage_kg_graph_exists` on every order from the Moore bound up, so Moore-tight cages confirm instantly; equivalence with the exact-girth cage definition is the strict monotonicity of n(k,g), Fu–Huang–Rodger 1997) added alongside — the recognizer includes the enumerator header, the reverse of the usual dependency. C++/CLI only, matching the kregular/partial_ktree/degenerate precedent (the extra k, g do not fit the Python factory signatures). Practical to about n = 14 for (3,5) and n = 16 for (3,6): the exact canonicalization branches heavily on sparse high-girth (locally tree-like) graphs, so GENREG's throughput (and the (3,7)-cage McGee, two orders above M(3,7) = 22) is out of reach; canonicalizing the complement was measured ~20x slower still
...
52 classes have one (tree, forest, caterpillar, unicyclic, halin, fullerene,
...
partial_ktree, degenerate, cage); the other 26 classes with a labeled enumerator do not. Convention: `notes/enum_alg.md`
...
- [x] **Polyhedral (unlabeled)** — plantri-style canonical construction path; A000944. Implemented via the generic canonical-augmentation route over planar graphs (`check_planar` + class-invariant edge-count window pruning ceil(3n/2) <= m <= 3n-6, full `check_polyhedral` at the last level), with plantri (Brinkmann–McKay 2007) referenced in the header; the survey's throughput does not apply
- [x] **Cubic Planar (unlabeled)** — plantri-style (generate triangulations and dualize); connected counts A005964. Implemented via the generic canonical-augmentation route (the cubic enumerator's degree-constrained search plus a `check_planar` prune per candidate child), with plantri (Brinkmann–McKay 2007) referenced in the header; the survey's throughput does not apply. The total counts allowing disconnected graphs (1, 1, 4, 10, 37, 146, ... for n = 4, 6, 8, ...) are not in the OEIS
...
- [x] **Strongly Regular (unlabeled)** — parameter-constrained backtracking + eigenvalue feasibility with isomorph rejection (McKay–Spence). Implemented as one canonical-augmentation search per feasible parameter tuple (the labeled enumerator's `srg_enumerate_params` prefilter), pruned by the k-regular deficit conditions plus a common-neighbor window per vertex pair (cn <= target and cn + min(def(u), def(v), r) >= target — necessary for induced subgraphs of an srg, exact at r = 0, so no recognizer call anywhere); no cross-tuple isomorph rejection is needed because an srg determines (k, lambda, mu) uniquely. Counts match the known classification (2, 1, 4, 0, 4, 3, 6, 0, 8, 1, 4, 6 for n = 4..15; no single OEIS sequence exists — the survey's A088741 is per-tuple). C++/CLI only, matching the labeled kregular/ktree/strongly_regular precedent
