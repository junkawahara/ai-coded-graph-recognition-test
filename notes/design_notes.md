...
- **The search runs on the vertices that carry a host edge, not on all of `V`.** The host matrix and `KiyomiUnoChordalState` are both Θ(k²) in the number `k` of search vertices while the output is bounded by `2^m`, so a legal input such as "10⁴ vertices, three edges" used to cost hundreds of megabytes although it has four subgraphs. The enumerator relabels the incident vertices to `1..k` (`k <= 2m`) and a translating callback maps the emitted edges back. The relabeling is monotone, so `kiyomi_uno_is_child_clique()` — which only compares vertex labels — and the output order are unchanged; the emitted `EnumeratedGraph::n` stays the host's `n` because the output is still spanning.
...
- **PEO_MATRIX and MCS_SEO must stay a differential pair.** Both eliminate simple vertices from a chordal graph, and the only difference is the simple test: `PEO_MATRIX` compares *every* pair of neighbours against the definition (the closed neighbourhoods of `N(v)` form a chain under inclusion), `MCS_SEO` sorts the neighbours by alive degree and compares only consecutive pairs. The optimization is what the pair exists to test, so if the two ever end up with the same test again (they did, until 2026-09-02), the property test comparing them checks nothing.
...
## Enumeration CLIs (src/*_enum_main.cpp)
- **Never print the size of an out-of-range result as a count.** The unlabeled enumerators return an empty result outside their bitmask range (`n >= 64`) and every one of these classes has a member on every `n` (the edgeless graph at least), so `std::cout << res.graphs.size()` turns "unsupported" into the false claim that the count is 0 — with exit status 0, indistinguishable from a real answer. `chordal_unlabeled_enum`, `split_unlabeled_enum` and `threshold_unlabeled_enum` reject an out-of-range `n`, a parse failure, a negative `n` and an unknown option with a message on stderr and exit 1; the other unlabeled CLIs still print `0`.
- **Print the count from a table or a counting pass, never from a materialized vector.** The established output format is "count, then one line per graph". Building the whole enumeration just to learn the count is what made `split_labeled_enum` (n = 8: 5,843,954 graphs) and `strongly_chordal_labeled_enum` (n = 7: 598,775) abort under a 64 MiB limit, although the callback APIs stream the same output in a few megabytes. Where the count is known in advance it comes from a table (`chordal_labeled_enum`, `split_labeled_enum`, `strongly_chordal_labeled_enum`) or a formula (`2^(n-1)` for `threshold_unlabeled_enum`); where it depends on the input, enumerate twice (`chordal_subgraph_enum`). Either way, compare the streamed count with the announced one afterwards and fail loudly on a mismatch, so a wrong table entry cannot become a wrong answer.
