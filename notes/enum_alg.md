...
| Implementation | `include/interval_enum.h` — **default is the dedicated Kiyomi--Kijima--Uno reverse search**. Rooted at K_n; the parent operation adds the edge from the largest-label non-universal vertex to its closest non-neighbor in the interval model. The old chordal vertex-addition + interval filter remains as `LEGACY_CHORDAL_FILTER` |
...
| Implementation | `include/split_enum.h` — default is the dedicated `KS_PARTITION_CANONICAL`. The old chordal subtree + split pruning is `LEGACY_CHORDAL_FILTER`. Streaming via callbacks supported |
...
| Implementation | `include/trivially_perfect_enum.h` (UVD_TREE) |
...
| Implementation | `include/strongly_chordal_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/ptolemaic_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/block_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/weakly_chordal_enum.h` — labeled exhaustive enumeration. The conventional method is `GENERIC_VERTEX_AUGMENTATION` |
...
| Implementation | `include/at_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/permutation_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/comparability_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/co_comparability_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/co_chordal_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/co_interval_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/cograph_enum.h` — labeled exhaustive enumeration (cotree construction) |
...
| Implementation | `include/distance_hereditary_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/circular_arc_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/proper_circular_arc_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/trapezoid_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/bipartite_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/bipartite_permutation_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/convex_bipartite_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/biconvex_bipartite_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/cochain_enum.h` — builds complements from the chain enumeration |
...
| Implementation | `include/planar_enum.h` — reverse search (vertex addition + planarity test) |
...
| Implementation | `include/outer_planar_enum.h` — reverse search (vertex addition + outerplanarity test) |
...
| Implementation | `include/series_parallel_enum.h` — reverse search (vertex addition + SP test) |
...
| Implementation | `include/cactus_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/line_graph_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/claw_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/diamond_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/perfect_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/three_leaf_power_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/tree_enum.h` — non-isomorphic free-tree enumeration (bottom-up recursive construction + centroid decomposition) |
...
| Implementation | `include/forest_enum.h` — non-isomorphic forest enumeration via integer partitions + tree composition |
...
| Implementation | `include/caterpillar_enum.h` — constructive enumeration (spine + leaf assignment) |
...
| Implementation | `include/ktree_enum.h` — labeled exhaustive enumeration (reverse search, k-clique restricted) |
...
| Implementation | `include/unicyclic_enum.h` — constructive enumeration (cycle + rooted-tree attachment) |
...
| Implementation | `include/biconnected_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/maximal_planar_enum.h` — labeled exhaustive enumeration (reverse search + planarity pruning + edge-count constraint) |
...
| Implementation | `include/triangle_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/eulerian_enum.h` — labeled exhaustive enumeration (cycle-space basis enumeration) |
...
| Implementation | `include/self_complementary_enum.h` — labeled exhaustive enumeration (complementing permutation) |
...
| Implementation | `include/kregular_enum.h` — labeled exhaustive enumeration (degree-constrained reverse search) |
...
| Implementation | `include/cubic_enum.h` — labeled exhaustive enumeration (reverse search with the degree-3 constraint) |
...
| Implementation | `include/strongly_regular_enum.h` — labeled exhaustive enumeration (parameter-constrained backtracking) |
...
| Implementation | `include/circle_enum.h` — labeled exhaustive enumeration (reverse search + DOW-backtracking recognition) |
...
| Implementation | `include/halin_enum.h` — non-isomorphic exhaustive enumeration (HI-tree + planar embedding + bracket-code normalization) |
...
| Implementation | `include/apex_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/cluster_enum.h` — labeled exhaustive enumeration (set-partition construction) |
...
| Implementation | `include/bull_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/parity_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/meyniel_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/even_hole_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/odd_hole_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/four_leaf_power_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/five_leaf_power_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/gem_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/p5_free_enum.h` — labeled exhaustive enumeration (reverse search) |
...
| Implementation | `include/polyhedral_enum.h` — labeled exhaustive enumeration (reverse search + planarity pruning + 3-connectivity test) |
...
| Implementation | `include/simple_quadrangulation_enum.h` — non-isomorphic exhaustive enumeration (dual approach: enumerate 4-regular planar 3-connected graphs → extract faces → build the dual) |
...
| Implementation | `include/cubic_planar_enum.h` — labeled exhaustive enumeration (reverse search, degree-3 + planarity pruning) |
| References | Brinkmann, McKay, MATCH 58, 2007 |
| PDF | `references/brinkmann2007_plantri.pdf` |
...
| Implementation | `include/fullerene_enum.h` — non-isomorphic enumeration (enumerate the dual triangulations by reverse search and dualize; isomorphism removal via a BFS canonical form) |
...
| Implementation | `include/snark_enum.h` — labeled exhaustive enumeration (reverse search, girth ≥ 5 pruning + bridgeless / cyclically 4-edge-connected / non-3-edge-colorable checks) |
...
| Implementation | `include/laman_enum.h` — labeled exhaustive enumeration (reverse search + incremental (2,3)-sparsity subset checks) |
...
| Implementation | `include/digraph_enum.h` — labeled exhaustive enumeration (constructive DFS, 4-way branch per pair) |
...
| Implementation | `include/tournament_enum.h` — labeled exhaustive enumeration (constructive DFS over all orientations of K_n) |
...
| Implementation | `include/poset_enum.h` — labeled exhaustive enumeration (pairwise DFS + incremental transitive closure) |
...
| Implementation | `include/proper_chordal.h` — recognition (Algorithms 1 + 2); `include/proper_chordal_enum.h` — labeled exhaustive enumeration (reverse search) |
