...
| 項目 | 内容 |
|------|------|
| 計算量 | 指数時間 (平面性は K5/K3,3 マイナー探索; メモ化付き) |
| アルゴリズム | (1) 平面性チェック (minor.h のマイナー探索)、(2) m = 3n - 6 (n ≥ 3) の確認 |
...
|------|------|
| 計算量 | 指数時間 (平面性は K5/K3,3 マイナー探索; メモ化付き) |
| アルゴリズム | (1) 全頂点の次数が 3 であることを確認、(2) 平面性チェック (minor.h のマイナー探索) |
...
|------|------|
| 計算量 | 指数時間 (平面性は K5/K3,3 マイナー探索; メモ化付き) |
| アルゴリズム | (1) 平面性チェック (minor.h のマイナー探索)、(2) 3-連結性チェック |
...
|------|------|
| 計算量 | 指数時間 (平面性は K5/K3,3 マイナー探索; メモ化付き) |
| アルゴリズム | (1) m = 2n - 4 の確認、(2) 三角形フリー確認、(3) 3-連結性チェック、(4) 平面性チェック (minor.h のマイナー探索) |
...
| 項目 | 内容 |
|------|------|
| 計算量 | 指数時間 (平面性はマイナー探索; 埋め込みは Tutte 座標) |
...
| 項目 | 内容 |
|------|------|
| 計算量 | 指数時間 (平面性はマイナー探索; n=20 で実測約 3 秒) |
