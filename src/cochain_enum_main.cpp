...
int main() {
    ...
    graph_recognition::CochainEnumerationResult res =
        graph_recognition::enumerate_cochain_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::CochainEnumeratedGraph& g = res.graphs[i];
