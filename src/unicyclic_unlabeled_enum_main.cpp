...
int main() {
    ...
    graph_recognition::UnicyclicUnlabeledEnumerationResult res =
        graph_recognition::enumerate_unicyclic_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::UnicyclicUnlabeledEnumeratedGraph& g = res.graphs[i];
