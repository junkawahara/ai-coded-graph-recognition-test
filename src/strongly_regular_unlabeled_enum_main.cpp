...
int main() {
    ...
    graph_recognition::StronglyRegularUnlabeledEnumerationResult res =
        graph_recognition::enumerate_strongly_regular_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::StronglyRegularUnlabeledEnumeratedGraph& g =
            res.graphs[i];
