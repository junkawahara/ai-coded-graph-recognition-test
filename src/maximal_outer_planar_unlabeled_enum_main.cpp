...
int main() {
    ...
    graph_recognition::MaximalOuterPlanarUnlabeledEnumerationResult res =
        graph_recognition::enumerate_maximal_outer_planar_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::MaximalOuterPlanarUnlabeledEnumeratedGraph& g =
            res.graphs[i];
