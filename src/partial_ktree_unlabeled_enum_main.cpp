...
int main(int argc, char** argv) {
    ...
    graph_recognition::PartialKTreeUnlabeledEnumerationResult res =
        graph_recognition::enumerate_partial_ktree_unlabeled_graphs(
            n, k, connected_only);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::PartialKTreeUnlabeledEnumeratedGraph& g =
            res.graphs[i];
