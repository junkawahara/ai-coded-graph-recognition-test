...
int main() {
    ...
    graph_recognition::ChainUnlabeledEnumerationResult res =
        graph_recognition::enumerate_chain_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::ChainUnlabeledEnumeratedGraph& g = res.graphs[i];
