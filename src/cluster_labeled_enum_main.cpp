...
int main() {
    ...
    graph_recognition::ClusterLabeledEnumerationResult res =
        graph_recognition::enumerate_cluster_labeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::ClusterLabeledEnumeratedGraph& g = res.graphs[i];
