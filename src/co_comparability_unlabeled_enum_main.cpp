...
int main() {
    ...
    graph_recognition::CoComparabilityUnlabeledEnumerationResult res =
        graph_recognition::enumerate_co_comparability_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::CoComparabilityUnlabeledEnumeratedGraph& g = res.graphs[i];
