...
int main() {
    ...
    graph_recognition::SelfComplementaryUnlabeledEnumerationResult res =
        graph_recognition::enumerate_self_complementary_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::SelfComplementaryUnlabeledEnumeratedGraph& g =
            res.graphs[i];
