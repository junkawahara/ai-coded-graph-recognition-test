...
int main() {
    ...
    graph_recognition::FullereneUnlabeledEnumerationResult res =
        graph_recognition::enumerate_fullerene_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::FullereneUnlabeledEnumeratedGraph& g = res.graphs[i];
