...
int main(int argc, char** argv) {
    ...
    graph_recognition::ThreeLeafPowerUnlabeledEnumerationResult res =
        graph_recognition::enumerate_three_leaf_power_unlabeled_graphs(
            n, connected_only);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::ThreeLeafPowerUnlabeledEnumeratedGraph& g =
            res.graphs[i];
