...
int main() {
    ...
    graph_recognition::CoIntervalUnlabeledEnumerationResult res =
        graph_recognition::enumerate_co_interval_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::CoIntervalUnlabeledEnumeratedGraph& g = res.graphs[i];
