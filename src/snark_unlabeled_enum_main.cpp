...
int main() {
    ...
    graph_recognition::SnarkUnlabeledEnumerationResult res =
        graph_recognition::enumerate_snark_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::SnarkUnlabeledEnumeratedGraph& g = res.graphs[i];
