...
int main() {
    ...
    graph_recognition::BiconnectedUnlabeledEnumerationResult res =
        graph_recognition::enumerate_biconnected_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::BiconnectedUnlabeledEnumeratedGraph& g = res.graphs[i];
