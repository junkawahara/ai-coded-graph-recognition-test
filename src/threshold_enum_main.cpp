...
int main() {
    ...
    graph_recognition::ThresholdEnumerationResult res =
        graph_recognition::enumerate_threshold_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::ThresholdEnumeratedGraph& g = res.graphs[i];
