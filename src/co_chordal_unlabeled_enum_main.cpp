...
int main() {
    ...
    graph_recognition::CoChordalUnlabeledEnumerationResult res =
        graph_recognition::enumerate_co_chordal_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::CoChordalUnlabeledEnumeratedGraph& g = res.graphs[i];
