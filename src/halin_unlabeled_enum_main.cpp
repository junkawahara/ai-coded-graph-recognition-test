...
int main() {
    ...
    graph_recognition::HalinUnlabeledEnumerationResult res =
        graph_recognition::enumerate_halin_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::HalinUnlabeledEnumeratedGraph& g = res.graphs[i];
