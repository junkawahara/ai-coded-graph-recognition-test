...
int main(int argc, char** argv) {
    ...
    graph_recognition::KRegularUnlabeledEnumerationResult res =
        graph_recognition::enumerate_kregular_unlabeled_graphs(n, k,
                                                               connected_only);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::KRegularUnlabeledEnumeratedGraph& g =
            res.graphs[i];
