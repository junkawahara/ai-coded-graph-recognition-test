...
int main() {
    ...
    graph_recognition::MaximalPlanarUnlabeledEnumerationResult res =
        graph_recognition::enumerate_maximal_planar_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::MaximalPlanarUnlabeledEnumeratedGraph& g =
            res.graphs[i];
