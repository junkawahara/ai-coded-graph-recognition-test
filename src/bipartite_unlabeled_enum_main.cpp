...
int main(int argc, char** argv) {
    ...
    graph_recognition::BipartiteUnlabeledEnumerationResult res =
        graph_recognition::enumerate_bipartite_unlabeled_graphs(n, connected_only);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::BipartiteUnlabeledEnumeratedGraph& g = res.graphs[i];
