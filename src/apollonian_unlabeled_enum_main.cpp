...
int main() {
    ...
    graph_recognition::ApollonianUnlabeledEnumerationResult res =
        graph_recognition::enumerate_apollonian_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::ApollonianUnlabeledEnumeratedGraph& g =
            res.graphs[i];
