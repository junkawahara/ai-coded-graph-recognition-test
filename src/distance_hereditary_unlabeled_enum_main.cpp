...
int main(int argc, char** argv) {
    ...
    graph_recognition::DistanceHereditaryUnlabeledEnumerationResult res =
        graph_recognition::enumerate_distance_hereditary_unlabeled_graphs(
            n, connected_only);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::DistanceHereditaryUnlabeledEnumeratedGraph& g =
            res.graphs[i];
