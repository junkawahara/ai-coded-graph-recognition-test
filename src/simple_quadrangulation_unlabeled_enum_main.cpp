...
int main() {
    ...
    graph_recognition::SimpleQuadrangulationUnlabeledEnumerationResult res =
        graph_recognition::enumerate_simple_quadrangulation_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::SimpleQuadEnumeratedGraph& g = res.graphs[i];
