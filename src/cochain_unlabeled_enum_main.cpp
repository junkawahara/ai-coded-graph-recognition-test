...
int main() {
    ...
    graph_recognition::CochainUnlabeledEnumerationResult res =
        graph_recognition::enumerate_cochain_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::CochainUnlabeledEnumeratedGraph& g = res.graphs[i];
