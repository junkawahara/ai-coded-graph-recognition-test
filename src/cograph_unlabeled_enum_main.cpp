...
int main(int argc, char** argv) {
    ...
    graph_recognition::CographUnlabeledEnumerationResult res =
        graph_recognition::enumerate_cograph_unlabeled_graphs(n, connected_only);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::CographUnlabeledEnumeratedGraph& g = res.graphs[i];
