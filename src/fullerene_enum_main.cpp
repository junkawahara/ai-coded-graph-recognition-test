...
int main() {
    ...
    graph_recognition::FullereneEnumerationResult res =
        graph_recognition::enumerate_fullerene_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::FullereneEnumeratedGraph& g = res.graphs[i];
