...
int main() {
    ...
    graph_recognition::ThresholdUnlabeledEnumerationResult res =
        graph_recognition::enumerate_threshold_unlabeled_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::ThresholdUnlabeledEnumeratedGraph& g = res.graphs[i];
