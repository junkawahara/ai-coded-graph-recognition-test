...
int main() {
    ...
    graph_recognition::ClusterEnumerationResult res =
        graph_recognition::enumerate_cluster_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::ClusterEnumeratedGraph& g = res.graphs[i];
