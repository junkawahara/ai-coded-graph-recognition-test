...
int main() {
    ...
    graph_recognition::UnicyclicEnumerationResult res =
        graph_recognition::enumerate_unicyclic_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::UnicyclicEnumeratedGraph& g = res.graphs[i];
