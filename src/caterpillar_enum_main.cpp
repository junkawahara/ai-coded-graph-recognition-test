...
int main() {
    ...
    graph_recognition::CaterpillarEnumerationResult res =
        graph_recognition::enumerate_caterpillar_graphs(n);

    std::cout << res.graphs.size() << "\n";
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        const graph_recognition::CaterpillarEnumeratedGraph& g = res.graphs[i];
