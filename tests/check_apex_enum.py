...
def is_apex(n, edges):
    """Check if graph is apex: exists vertex v such that G-v is planar."""
    if n <= 5:
        return True
    for v in range(1, n + 1):
        # Build G-v with renumbered vertices
        remaining = [u for u in range(1, n + 1) if u != v]
        new_id = {}
        ...
        for u, w in edges:
            if u != v and w != v:
                sub_edges.append((new_id[u], new_id[w]))
        if is_planar(n - 1, sub_edges):
            return True
    return False
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_apex_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_apex(n, edges):
            print("non-apex graph found:", edges)
            return 1
