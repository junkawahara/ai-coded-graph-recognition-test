...
def is_gem_free(n, edges):
    """Check if graph is gem-free."""
    adj = [set() for _ in range(n + 1)]
    for u, v in edges:
        adj[u].add(v)
        adj[v].add(u)
    # gem: P4 {a,b,c,d} + universal vertex v
    # edges: va, vb, vc, vd, ab, bc, cd
    # non-edges: ac, ad, bd
    for subset in combinations(range(1, n + 1), 5):
        for v, a, b, c, d in permutations(subset):
            if (a in adj[v] and b in adj[v] and c in adj[v] and d in adj[v] and
                a in adj[b] and b in adj[c] and c in adj[d] and
                a not in adj[c] and a not in adj[d] and b not in adj[d]):
                return False
    return True
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_gem_free_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_gem_free(n, edges):
            print("non-gem-free graph found:", edges)
            return 1
