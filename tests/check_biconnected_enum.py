...
def is_biconnected(n, edges):
    """Check if graph on n vertices with given edges is 2-connected.

    2-connected: n >= 3, connected, no cut vertex.
    Uses vertex-deletion approach: for each vertex v, check G-v is connected.
    """
    if n < 3:
        return False
    ...
    # Check no cut vertex: for each vertex, G-v must be connected
    for rem in range(1, n + 1):
        remaining = [v for v in range(1, n + 1) if v != rem]
        vis = set()
        stack = [remaining[0]]
        while stack:
            v = stack.pop()
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_biconnected_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_biconnected(n, edges):
            print("non-biconnected graph found:", edges)
            return 1
