...
def is_unicyclic(n, edges):
    """Check if graph is unicyclic: connected, n vertices, n edges."""
    if n < 3:
        return False
    if len(edges) != n:
        return False
    ...
    # Connected graph with n vertices and n edges has exactly one cycle
    return True


def main():
    if len(sys.argv) != 4:
        print("usage: check_unicyclic_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_unicyclic(n, edges):
            print("non-unicyclic graph found:", edges)
            return 1
