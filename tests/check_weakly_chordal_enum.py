...
def _has_hole_ge5(n, adj):
    """Check if graph has an induced cycle of length >= 5."""
...
def is_weakly_chordal(n, edges):
    """Check if graph is weakly chordal (no hole or anti-hole of length >= 5)."""
    adj = [set() for _ in range(n + 1)]
    for u, v in edges:
        adj[u].add(v)
        adj[v].add(u)
    if _has_hole_ge5(n, adj):
        return False
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_weakly_chordal_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_weakly_chordal(n, edges):
            print("non-weakly-chordal graph found:", edges)
            return 1
