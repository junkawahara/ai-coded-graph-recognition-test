...
using graph_recognition::BiconvexBipartiteEnumerationResult;
using graph_recognition::enumerate_biconvex_bipartite_graphs_reverse_search;
...
TEST(BiconvexBipartiteEnumProperty, EnumerationAgreesWithBruteForce) {
    ...
    for (int n = 1; n <= max_n; ++n) {
        ...
        for (int gi = 0; gi < bcb_by_enum; ++gi) {
            ...
            Graph g(n, eg.edges);
            BiconvexBipartiteResult res = check_biconvex_bipartite(g);
            ASSERT_TRUE(res.is_biconvex_bipartite)
                << "n=" << n << " graph#" << gi << " is not biconvex bipartite";
        }

        // Compare counts
        ASSERT_EQ(bcb_by_filter, bcb_by_enum) << "n=" << n;
    }
}
