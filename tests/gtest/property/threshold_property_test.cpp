 ...
 */
bool bf_is_threshold(int n, const std::vector<std::pair<int, int>>& edges) {
...
/** @brief ランダム閾値グラフを生成 */
std::vector<std::pair<int, int>> gen_random_threshold(int n) {
...
TEST(ThresholdProperty, RandomTrialsAgreeWithBruteForce) {
    std::srand(42);
    ...
    for (int trial = 0; trial < 3000; ++trial) {
        ...
        ASSERT_EQ(r1.is_threshold, r2.is_threshold)
            << "OLD vs FAST trial=" << trial << " n=" << n << " m=" << edges.size();
        ASSERT_EQ(r1.is_threshold, bf)
