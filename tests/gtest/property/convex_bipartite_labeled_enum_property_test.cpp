...
TEST(ConvexBipartiteLabeledEnumProperty, EnumerationAgreesWithBruteForce) {
    int max_n = 6;

    for (int n = 1; n <= max_n; ++n) {
        auto enum_res = graph_recognition::enumerate_convex_bipartite_labeled_graphs_reverse_search(n);
        expect_enum_equals_bruteforce(
            n,
            [](const Graph& g) {
                return graph_recognition::check_convex_bipartite(g).is_convex_bipartite;
            },
            enum_res.graphs, "convex_bipartite");
    }
}
