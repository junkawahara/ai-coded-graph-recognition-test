...
TEST(OuterPlanarLabeledEnumProperty, EnumerationAgreesWithBruteForce) {
    int max_n = 6;

    for (int n = 1; n <= max_n; ++n) {
        auto enum_res = graph_recognition::enumerate_outer_planar_labeled_graphs_reverse_search(n);
        expect_enum_equals_bruteforce(
            n,
            [](const Graph& g) {
                return graph_recognition::check_outer_planar(g).is_outer_planar;
            },
            enum_res.graphs, "outer_planar");
    }
}
