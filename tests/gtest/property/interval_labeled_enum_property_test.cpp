...
TEST(IntervalLabeledEnumProperty, EnumerationAgreesWithBruteForce) {
    int max_n = 7;

    for (int n = 1; n <= max_n; ++n) {
        ChordalLabeledEnumerationResult enumres =
        ...
        // interval_labeled_enum_full_property_test が全グラフ総当たりに対して検証
        // 済み。ここでは n=7 まで件数を照合してカバレッジを広げる)。
        IntervalLabeledEnumerationResult ienum = enumerate_interval_labeled_graphs_reverse_search(n);
