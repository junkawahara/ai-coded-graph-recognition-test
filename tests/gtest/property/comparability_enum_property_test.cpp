...
TEST(ComparabilityEnumProperty, EnumerationAgreesWithBruteForce) {
    int max_n = 6;
    ...
    for (int n = 1; n <= max_n; ++n) {
        ...
        for (int gi = 0; gi < comp_by_enum; ++gi) {
            ...
            Graph g(n, eg.edges);
            ComparabilityResult res = check_comparability(g);
            ASSERT_TRUE(res.is_comparability)
                << "n=" << n << " graph#" << gi << " is not comparability";
        }

        // Compare counts
        ASSERT_EQ(comp_by_filter, comp_by_enum) << "n=" << n;
    }
}
