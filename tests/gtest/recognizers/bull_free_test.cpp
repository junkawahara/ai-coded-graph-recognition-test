...
using graph_recognition::BullFreeAlgorithm;
...
TEST_P(BullFreeTest, MatchesExpected) {
    ...
    const BullFreeAlgorithm algos[] = {BullFreeAlgorithm::BRUTE,
                                       BullFreeAlgorithm::TRIANGLE_SEARCH};
    for (size_t i = 0; i < 2; ++i) {
        BullFreeResult r = check_bull_free(g, algos[i]);
        ASSERT_EQ(r.is_bull_free, exp == "YES") << "case=" << stem << " algo=" << i;
        if (r.is_bull_free) continue;
        EXPECT_EQ(r.obstruction.kind, ObstructionKind::BULL)
            << "case=" << stem << " algo=" << i;
