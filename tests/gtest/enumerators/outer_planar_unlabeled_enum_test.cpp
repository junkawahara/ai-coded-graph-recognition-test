...
using graph_recognition::OuterPlanarUnlabeledEnumerationResult;
using graph_recognition::check_outer_planar;
using graph_recognition::enumerate_outer_planar_labeled_graphs_reverse_search;
using graph_recognition::enumerate_outer_planar_unlabeled_graphs;
...
const char* kDir = "outer_planar_unlabeled_enum";
...
class OuterPlanarUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(OuterPlanarUnlabeledEnumTest, CountAndAllValid) {
    ...
    OuterPlanarUnlabeledEnumerationResult res = enumerate_outer_planar_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 1150 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
        ...
        EXPECT_TRUE(check_outer_planar(g).is_outer_planar)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, OuterPlanarUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected outerplanar graph counts (A111563),
// with every graph connected, valid and pairwise non-isomorphic.
TEST(OuterPlanarUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 5, 13, 46, 172};
    for (int n = 1; n <= 7; ++n) {
        OuterPlanarUnlabeledEnumerationResult res =
            enumerate_outer_planar_unlabeled_graphs(n, true);
...
TEST(OuterPlanarUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::OuterPlanarLabeledEnumerationResult labeled =
            enumerate_outer_planar_labeled_graphs_reverse_search(n);
        ...
        OuterPlanarUnlabeledEnumerationResult unlabeled =
