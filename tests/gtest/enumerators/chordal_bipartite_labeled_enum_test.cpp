...
using graph_recognition::ChordalBipartiteLabeledEnumerationResult;
using graph_recognition::check_chordal_bipartite;
using graph_recognition::enumerate_chordal_bipartite_labeled_graphs_reverse_search;
...
const char* kDir = "chordal_bipartite_labeled_enum";

class ChordalBipartiteLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ChordalBipartiteLabeledEnumTest, CountAndAllValid) {
    ...
    ChordalBipartiteLabeledEnumerationResult res = enumerate_chordal_bipartite_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_chordal_bipartite(g).is_chordal_bipartite) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ChordalBipartiteLabeledEnumTest,
