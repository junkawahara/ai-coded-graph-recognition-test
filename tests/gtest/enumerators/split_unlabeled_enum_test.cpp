...
using graph_recognition::SplitUnlabeledEnumerationResult;
using graph_recognition::Graph;
using graph_recognition::check_split;
using graph_recognition::enumerate_split_labeled_graphs_reverse_search;
using graph_recognition::enumerate_split_unlabeled_graphs;
...
const char* kDir = "split_unlabeled_enum";
...
class SplitUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(SplitUnlabeledEnumTest, CountAndAllValid) {
    ...
    SplitUnlabeledEnumerationResult res = enumerate_split_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 557 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
        ...
        EXPECT_TRUE(check_split(g).is_split)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, SplitUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected split graph counts (the first
// differences of A048194),
// with every graph connected, valid and pairwise non-isomorphic.
TEST(SplitUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 5, 12, 35, 108};
    for (int n = 1; n <= 7; ++n) {
        SplitUnlabeledEnumerationResult res =
            enumerate_split_unlabeled_graphs(n, true);
...
TEST(SplitUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::SplitLabeledEnumerationResult labeled =
        ...
        SplitUnlabeledEnumerationResult unlabeled =
