...
using graph_recognition::HalinUnlabeledEnumerationResult;
using graph_recognition::check_halin;
using graph_recognition::enumerate_halin_unlabeled_graphs;
...
const char* kDir = "halin_unlabeled_enum";

class HalinUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(HalinUnlabeledEnumTest, CountAndAllValid) {
    ...
    HalinUnlabeledEnumerationResult res = enumerate_halin_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_halin(g).is_halin) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, HalinUnlabeledEnumTest,
