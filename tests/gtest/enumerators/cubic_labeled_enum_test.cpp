...
using graph_recognition::CubicLabeledEnumerationResult;
...
const char* kDir = "cubic_labeled_enum";

class CubicLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CubicLabeledEnumTest, CountAndAllValid) {
    ...
    CubicLabeledEnumerationResult res = enumerate_cubic_labeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_cubic(g).is_cubic) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CubicLabeledEnumTest,
