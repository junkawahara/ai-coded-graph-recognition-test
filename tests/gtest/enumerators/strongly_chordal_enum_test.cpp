...
using graph_recognition::StronglyChordalEnumAlgorithm;
using graph_recognition::StronglyChordalEnumerationResult;
using graph_recognition::check_strongly_chordal;
using graph_recognition::enumerate_strongly_chordal_graphs_reverse_search;
using graph_recognition::enumerate_strongly_chordal_graphs_reverse_search_cb;
...
const char* kDir = "strongly_chordal_enum";

class StronglyChordalEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(StronglyChordalEnumTest, CountAndAllValid) {
    ...
    StronglyChordalEnumerationResult res = enumerate_strongly_chordal_graphs_reverse_search(n);
...
INSTANTIATE_TEST_SUITE_P(
    AllCases, StronglyChordalEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

std::set<std::vector<std::pair<int, int>>> graph_set(
    const StronglyChordalEnumerationResult& result) {
...
TEST(StronglyChordalEnumKiyomiTest, EnumeratesSameSetsAsLegacyFilter) {
    for (int n = 0; n <= 6; ++n) {
        const StronglyChordalEnumerationResult dedicated =
            enumerate_strongly_chordal_graphs_reverse_search(
                n, StronglyChordalEnumAlgorithm::KIYOMI_EDGE_ADDITION);
        const StronglyChordalEnumerationResult legacy =
            enumerate_strongly_chordal_graphs_reverse_search(
                n, StronglyChordalEnumAlgorithm::LEGACY_CHORDAL_FILTER);
...
TEST(StronglyChordalEnumKiyomiTest, ExhaustiveSimpleGraphsAgreeThroughSixVertices) {
    for (int n = 0; n <= 6; ++n) {
        ...
        const StronglyChordalEnumerationResult dedicated =
            enumerate_strongly_chordal_graphs_reverse_search(n);
        EXPECT_EQ(graph_set(dedicated), brute_force) << "n=" << n;
    }
}

TEST(StronglyChordalEnumKiyomiTest, StreamsSameGraphsAsMaterializingApi) {
    for (int n = 0; n <= 5; ++n) {
        const StronglyChordalEnumerationResult materialized =
            enumerate_strongly_chordal_graphs_reverse_search(n);
        std::vector<graph_recognition::EnumeratedGraph> streamed;
        enumerate_strongly_chordal_graphs_reverse_search_cb(
            n,
...
TEST(StronglyChordalEnumKiyomiTest, CanonicalParentDeletionPreservesClass) {
    for (int n = 1; n <= 6; ++n) {
        const StronglyChordalEnumerationResult result =
            enumerate_strongly_chordal_graphs_reverse_search(n);
        for (std::size_t i = 0; i < result.graphs.size(); ++i) {
...
TEST(StronglyChordalEnumKiyomiTest, RejectsSunAsAChild) {
