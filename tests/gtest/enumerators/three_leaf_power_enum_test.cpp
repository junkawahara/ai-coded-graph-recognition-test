...
using graph_recognition::ThreeLeafPowerEnumerationResult;
using graph_recognition::check_three_leaf_power;
using graph_recognition::enumerate_three_leaf_power_graphs_reverse_search;
...
const char* kDir = "three_leaf_power_enum";

class ThreeLeafPowerEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ThreeLeafPowerEnumTest, CountAndAllValid) {
    ...
    ThreeLeafPowerEnumerationResult res = enumerate_three_leaf_power_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_three_leaf_power(g).is_three_leaf_power) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ThreeLeafPowerEnumTest,
