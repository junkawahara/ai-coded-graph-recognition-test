...
using graph_recognition::CircleLabeledEnumerationResult;
...
const char* kDir = "circle_labeled_enum";

class CircleLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CircleLabeledEnumTest, CountAndAllValid) {
    ...
    CircleLabeledEnumerationResult res = enumerate_circle_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_circle(g).is_circle) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CircleLabeledEnumTest,
