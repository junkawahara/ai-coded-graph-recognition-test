...
using graph_recognition::LineGraphLabeledEnumerationResult;
...
const char* kDir = "line_graph_labeled_enum";

class LineGraphLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(LineGraphLabeledEnumTest, CountAndAllValid) {
    ...
    LineGraphLabeledEnumerationResult res = enumerate_line_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_line_graph(g).is_line_graph) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, LineGraphLabeledEnumTest,
