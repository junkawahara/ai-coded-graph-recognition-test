...
using graph_recognition::ComparabilityLabeledEnumerationResult;
...
const char* kDir = "comparability_labeled_enum";

class ComparabilityLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ComparabilityLabeledEnumTest, CountAndAllValid) {
    ...
    ComparabilityLabeledEnumerationResult res = enumerate_comparability_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_comparability(g).is_comparability) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ComparabilityLabeledEnumTest,
