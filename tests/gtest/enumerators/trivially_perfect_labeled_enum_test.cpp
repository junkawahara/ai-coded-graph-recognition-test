...
using graph_recognition::TriviallyPerfectLabeledEnumerationResult;
...
const char* kDir = "trivially_perfect_labeled_enum";

class TriviallyPerfectLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(TriviallyPerfectLabeledEnumTest, CountAndAllValid) {
    ...
    TriviallyPerfectLabeledEnumerationResult res = enumerate_trivially_perfect_labeled_graphs_uvd(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_trivially_perfect(g).is_trivially_perfect) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, TriviallyPerfectLabeledEnumTest,
