...
using graph_recognition::EvenHoleFreeLabeledEnumerationResult;
using graph_recognition::check_even_hole_free;
using graph_recognition::enumerate_even_hole_free_labeled_graphs_reverse_search;
...
const char* kDir = "even_hole_free_labeled_enum";

class EvenHoleFreeLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(EvenHoleFreeLabeledEnumTest, CountAndAllValid) {
    ...
    EvenHoleFreeLabeledEnumerationResult res = enumerate_even_hole_free_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_even_hole_free(g).is_even_hole_free) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, EvenHoleFreeLabeledEnumTest,
