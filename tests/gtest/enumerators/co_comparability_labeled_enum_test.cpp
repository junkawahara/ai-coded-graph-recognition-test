...
using graph_recognition::CoComparabilityLabeledEnumerationResult;
using graph_recognition::check_co_comparability;
using graph_recognition::enumerate_co_comparability_labeled_graphs_reverse_search;
...
const char* kDir = "co_comparability_labeled_enum";

class CoComparabilityLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CoComparabilityLabeledEnumTest, CountAndAllValid) {
    ...
    CoComparabilityLabeledEnumerationResult res = enumerate_co_comparability_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_co_comparability(g).is_co_comparability) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CoComparabilityLabeledEnumTest,
