...
using graph_recognition::OddHoleFreeLabeledEnumerationResult;
using graph_recognition::check_odd_hole_free;
using graph_recognition::enumerate_odd_hole_free_labeled_graphs_reverse_search;
...
const char* kDir = "odd_hole_free_labeled_enum";

class OddHoleFreeLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(OddHoleFreeLabeledEnumTest, CountAndAllValid) {
    ...
    OddHoleFreeLabeledEnumerationResult res = enumerate_odd_hole_free_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_odd_hole_free(g).is_odd_hole_free) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, OddHoleFreeLabeledEnumTest,
