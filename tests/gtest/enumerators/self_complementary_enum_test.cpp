...
using graph_recognition::SelfComplementaryEnumerationResult;
using graph_recognition::check_self_complementary;
using graph_recognition::enumerate_self_complementary_graphs;
...
const char* kDir = "self_complementary_enum";

class SelfComplementaryEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(SelfComplementaryEnumTest, CountAndAllValid) {
    ...
    SelfComplementaryEnumerationResult res = enumerate_self_complementary_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_self_complementary(g).is_self_complementary) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, SelfComplementaryEnumTest,
