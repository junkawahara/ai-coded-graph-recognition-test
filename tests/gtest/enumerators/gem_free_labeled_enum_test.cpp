...
using graph_recognition::GemFreeLabeledEnumerationResult;
using graph_recognition::check_gem_free;
using graph_recognition::enumerate_gem_free_labeled_graphs_reverse_search;
...
const char* kDir = "gem_free_labeled_enum";

class GemFreeLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(GemFreeLabeledEnumTest, CountAndAllValid) {
    ...
    GemFreeLabeledEnumerationResult res = enumerate_gem_free_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_gem_free(g).is_gem_free) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, GemFreeLabeledEnumTest,
