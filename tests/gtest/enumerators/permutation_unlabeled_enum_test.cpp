...
using graph_recognition::PermutationUnlabeledEnumerationResult;
using graph_recognition::check_permutation;
using graph_recognition::enumerate_permutation_labeled_graphs_reverse_search;
using graph_recognition::enumerate_permutation_unlabeled_graphs;
...
const char* kDir = "permutation_unlabeled_enum";
...
class PermutationUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(PermutationUnlabeledEnumTest, CountAndAllValid) {
    ...
    PermutationUnlabeledEnumerationResult res = enumerate_permutation_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 5699 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
        ...
        EXPECT_TRUE(check_permutation(g).is_permutation)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, PermutationUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected permutation graph counts, with
// every graph connected, valid and pairwise non-isomorphic.
TEST(PermutationUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 6, 20, 99, 600};
    for (int n = 1; n <= 7; ++n) {
        PermutationUnlabeledEnumerationResult res =
            enumerate_permutation_unlabeled_graphs(n, true);
...
// labeled enumerator produces. Capped at n = 6 (the labeled enumerator runs
// one recognition per candidate subset, and canonicalization is 6! per graph).
TEST(PermutationUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::PermutationLabeledEnumerationResult labeled =
            enumerate_permutation_labeled_graphs_reverse_search(n);
        ...
        PermutationUnlabeledEnumerationResult unlabeled =
