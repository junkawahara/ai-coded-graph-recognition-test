...
using graph_recognition::ConvexBipartiteLabeledEnumerationResult;
using graph_recognition::check_convex_bipartite;
using graph_recognition::enumerate_convex_bipartite_labeled_graphs_reverse_search;
...
const char* kDir = "convex_bipartite_labeled_enum";

class ConvexBipartiteLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ConvexBipartiteLabeledEnumTest, CountAndAllValid) {
    ...
    ConvexBipartiteLabeledEnumerationResult res = enumerate_convex_bipartite_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_convex_bipartite(g).is_convex_bipartite) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ConvexBipartiteLabeledEnumTest,
