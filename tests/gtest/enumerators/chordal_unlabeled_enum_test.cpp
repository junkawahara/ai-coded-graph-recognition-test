...
using graph_recognition::ChordalUnlabeledEnumerationResult;
using graph_recognition::Graph;
using graph_recognition::check_chordal;
using graph_recognition::enumerate_chordal_labeled_graphs_reverse_search;
using graph_recognition::enumerate_chordal_unlabeled_graphs;
...
const char* kDir = "chordal_unlabeled_enum";
...
class ChordalUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ChordalUnlabeledEnumTest, CountAndAllValid) {
    ...
    ChordalUnlabeledEnumerationResult res = enumerate_chordal_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 2119 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
        ...
        EXPECT_TRUE(check_chordal(g).is_chordal)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ChordalUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected chordal graph counts (A048192),
// with every graph connected, valid and pairwise non-isomorphic.
TEST(ChordalUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 5, 15, 58, 272};
    for (int n = 1; n <= 7; ++n) {
        ChordalUnlabeledEnumerationResult res =
            enumerate_chordal_unlabeled_graphs(n, true);
...
TEST(ChordalUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::ChordalLabeledEnumerationResult labeled =
        ...
        ChordalUnlabeledEnumerationResult unlabeled =
