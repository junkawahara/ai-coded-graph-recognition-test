...
using graph_recognition::CircleUnlabeledEnumerationResult;
using graph_recognition::Graph;
using graph_recognition::check_circle;
using graph_recognition::enumerate_circle_labeled_graphs_reverse_search;
using graph_recognition::enumerate_circle_unlabeled_graphs;
...
const char* kDir = "circle_unlabeled_enum";
...
class CircleUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CircleUnlabeledEnumTest, CountAndAllValid) {
    ...
    CircleUnlabeledEnumerationResult res = enumerate_circle_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 9497 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
...
INSTANTIATE_TEST_SUITE_P(
    AllCases, CircleUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected circle graph counts (A156808),
// with every graph connected, valid and pairwise non-isomorphic.
TEST(CircleUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 6, 21, 110, 789};
    for (int n = 1; n <= 7; ++n) {
        CircleUnlabeledEnumerationResult res =
            enumerate_circle_unlabeled_graphs(n, true);
...
TEST(CircleUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::CircleLabeledEnumerationResult labeled =
            enumerate_circle_labeled_graphs_reverse_search(n);
        ...
        CircleUnlabeledEnumerationResult unlabeled =
