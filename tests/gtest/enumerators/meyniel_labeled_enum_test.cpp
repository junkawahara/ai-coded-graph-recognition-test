...
using graph_recognition::MeynielLabeledEnumerationResult;
using graph_recognition::check_meyniel;
using graph_recognition::enumerate_meyniel_labeled_graphs_reverse_search;
...
const char* kDir = "meyniel_labeled_enum";

class MeynielLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(MeynielLabeledEnumTest, CountAndAllValid) {
    ...
    MeynielLabeledEnumerationResult res = enumerate_meyniel_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_meyniel(g).is_meyniel) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, MeynielLabeledEnumTest,
