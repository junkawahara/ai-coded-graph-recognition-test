...
using graph_recognition::PerfectLabeledEnumerationResult;
using graph_recognition::check_perfect;
using graph_recognition::enumerate_perfect_labeled_graphs_reverse_search;
...
const char* kDir = "perfect_labeled_enum";

class PerfectLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(PerfectLabeledEnumTest, CountAndAllValid) {
    ...
    PerfectLabeledEnumerationResult res = enumerate_perfect_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_perfect(g).is_perfect) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, PerfectLabeledEnumTest,
