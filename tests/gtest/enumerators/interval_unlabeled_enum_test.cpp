...
using graph_recognition::IntervalUnlabeledEnumerationResult;
using graph_recognition::check_interval;
using graph_recognition::enumerate_interval_labeled_graphs_reverse_search;
using graph_recognition::enumerate_interval_unlabeled_graphs;
...
const char* kDir = "interval_unlabeled_enum";
...
class IntervalUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(IntervalUnlabeledEnumTest, CountAndAllValid) {
    ...
    IntervalUnlabeledEnumerationResult res = enumerate_interval_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 1807 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
        ...
        EXPECT_TRUE(check_interval(g).is_interval)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, IntervalUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected interval graph counts (A005976),
// with every graph connected, valid and pairwise non-isomorphic.
TEST(IntervalUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 5, 15, 56, 250};
    for (int n = 1; n <= 7; ++n) {
        IntervalUnlabeledEnumerationResult res =
            enumerate_interval_unlabeled_graphs(n, true);
...
TEST(IntervalUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::IntervalLabeledEnumerationResult labeled =
        ...
        IntervalUnlabeledEnumerationResult unlabeled =
