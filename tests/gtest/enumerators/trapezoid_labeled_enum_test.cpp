...
using graph_recognition::TrapezoidLabeledEnumerationResult;
using graph_recognition::check_trapezoid;
using graph_recognition::enumerate_trapezoid_labeled_graphs_reverse_search;
...
const char* kDir = "trapezoid_labeled_enum";

class TrapezoidLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(TrapezoidLabeledEnumTest, CountAndAllValid) {
    ...
    TrapezoidLabeledEnumerationResult res = enumerate_trapezoid_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_trapezoid(g).is_trapezoid) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, TrapezoidLabeledEnumTest,
