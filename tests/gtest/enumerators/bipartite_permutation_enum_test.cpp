...
const char* kDir = "bipartite_permutation_enum";

class BipartitePermutationEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(BipartitePermutationEnumTest, CountAndAllValid) {
    ...
    BipartitePermutationEnumerationResult res = enumerate_bipartite_permutation_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_bipartite_permutation(g).is_bipartite_permutation) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, BipartitePermutationEnumTest,
