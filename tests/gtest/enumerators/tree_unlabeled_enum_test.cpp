...
using graph_recognition::TreeUnlabeledEnumerationResult;
using graph_recognition::check_tree;
using graph_recognition::enumerate_tree_unlabeled_graphs;
...
const char* kDir = "tree_unlabeled_enum";

class TreeUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(TreeUnlabeledEnumTest, CountAndAllValid) {
    ...
    TreeUnlabeledEnumerationResult res = enumerate_tree_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_tree(g).is_tree) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, TreeUnlabeledEnumTest,
