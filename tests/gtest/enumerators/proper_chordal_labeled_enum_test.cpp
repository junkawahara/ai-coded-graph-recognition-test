...
using graph_recognition::ProperChordalLabeledEnumAlgorithm;
using graph_recognition::ProperChordalLabeledEnumerationResult;
using graph_recognition::check_proper_chordal;
using graph_recognition::enumerate_proper_chordal_labeled_graphs_reverse_search;
using graph_recognition::enumerate_proper_chordal_labeled_graphs_reverse_search_cb;
...
const char* kDir = "proper_chordal_labeled_enum";

class ProperChordalLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

std::set<std::vector<std::pair<int, int>>> graph_set(
    const ProperChordalLabeledEnumerationResult& result) {
...
TEST_P(ProperChordalLabeledEnumTest, CountAndAllValid) {
    ...
    ProperChordalLabeledEnumerationResult res = enumerate_proper_chordal_labeled_graphs_reverse_search(n);
...
INSTANTIATE_TEST_SUITE_P(
    AllCases, ProperChordalLabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

TEST(ProperChordalLabeledEnumEdgeSearchTest, EnumeratesSameSetsAsLegacyFilter) {
    for (int n = 0; n <= 5; ++n) {
        const ProperChordalLabeledEnumerationResult dedicated =
            enumerate_proper_chordal_labeled_graphs_reverse_search(
                n, ProperChordalLabeledEnumAlgorithm::INDIFFERENCE_EDGE_ADDITION);
        const ProperChordalLabeledEnumerationResult legacy =
            enumerate_proper_chordal_labeled_graphs_reverse_search(
                n, ProperChordalLabeledEnumAlgorithm::LEGACY_CHORDAL_FILTER);
...
TEST(ProperChordalLabeledEnumEdgeSearchTest,
    ...
    for (int n = 0; n <= 6; ++n) {
        ...
        const ProperChordalLabeledEnumerationResult dedicated =
            enumerate_proper_chordal_labeled_graphs_reverse_search(n);
        EXPECT_EQ(graph_set(dedicated), brute_force) << "n=" << n;
    }
}

TEST(ProperChordalLabeledEnumEdgeSearchTest, StreamsSameGraphsAsMaterializingApi) {
    for (int n = 0; n <= 5; ++n) {
        const ProperChordalLabeledEnumerationResult materialized =
            enumerate_proper_chordal_labeled_graphs_reverse_search(n);
        std::vector<graph_recognition::EnumeratedGraph> streamed;
        enumerate_proper_chordal_labeled_graphs_reverse_search_cb(
            n,
...
TEST(ProperChordalLabeledEnumEdgeSearchTest, CanonicalParentDeletionPreservesClass) {
    for (int n = 1; n <= 6; ++n) {
        const ProperChordalLabeledEnumerationResult result =
            enumerate_proper_chordal_labeled_graphs_reverse_search(n);
        for (std::size_t i = 0; i < result.graphs.size(); ++i) {
