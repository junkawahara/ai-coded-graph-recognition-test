...
using graph_recognition::ClawFreeLabeledEnumerationResult;
using graph_recognition::check_claw_free;
using graph_recognition::enumerate_claw_free_labeled_graphs_reverse_search;
...
const char* kDir = "claw_free_labeled_enum";

class ClawFreeLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ClawFreeLabeledEnumTest, CountAndAllValid) {
    ...
    ClawFreeLabeledEnumerationResult res = enumerate_claw_free_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_claw_free(g).is_claw_free) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ClawFreeLabeledEnumTest,
