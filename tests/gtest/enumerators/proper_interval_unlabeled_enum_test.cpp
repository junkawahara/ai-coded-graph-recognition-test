...
const char* kDir = "proper_interval_unlabeled_enum";
...
class ProperIntervalUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ProperIntervalUnlabeledEnumTest, CountAndAllValid) {
    ...
    ProperIntervalUnlabeledEnumerationResult res =
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_proper_interval(g).is_proper_interval)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ProperIntervalUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce A007123 with every graph connected, valid and
// pairwise non-isomorphic.
TEST(ProperIntervalUnlabeledEnumConnectedTest, MatchesA007123) {
    const int expected[] = {1, 1, 2, 4, 10, 26, 76, 232};
    for (int n = 1; n <= 8; ++n) {
        ProperIntervalUnlabeledEnumerationResult res =
            enumerate_proper_interval_unlabeled_graphs(n, true);
...
// labeled enumerator produces. Capped at n = 6 (9653 labeled graphs x 720
// permutations); n = 7 would be 182065 x 5040.
TEST(ProperIntervalUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        ...
        ProperIntervalUnlabeledEnumerationResult unlabeled =
            enumerate_proper_interval_unlabeled_graphs(n);
