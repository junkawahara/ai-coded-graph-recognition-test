...
using graph_recognition::CubicPlanarLabeledEnumerationResult;
...
const char* kDir = "cubic_planar_labeled_enum";

class CubicPlanarLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CubicPlanarLabeledEnumTest, CountAndAllValid) {
    ...
    CubicPlanarLabeledEnumerationResult res = enumerate_cubic_planar_labeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_cubic_planar(g).is_cubic_planar) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CubicPlanarLabeledEnumTest,
