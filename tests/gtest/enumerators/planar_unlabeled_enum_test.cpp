...
using graph_recognition::PlanarUnlabeledEnumerationResult;
using graph_recognition::Graph;
using graph_recognition::check_planar;
using graph_recognition::enumerate_planar_labeled_graphs_reverse_search;
using graph_recognition::enumerate_planar_unlabeled_graphs;
...
const char* kDir = "planar_unlabeled_enum";
...
class PlanarUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(PlanarUnlabeledEnumTest, CountAndAllValid) {
    ...
    PlanarUnlabeledEnumerationResult res = enumerate_planar_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 6966 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
        ...
        EXPECT_TRUE(check_planar(g).is_planar)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, PlanarUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected planar graph counts (A003094),
// with every graph connected, valid and pairwise non-isomorphic.
TEST(PlanarUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 6, 20, 99, 646};
    for (int n = 1; n <= 7; ++n) {
        PlanarUnlabeledEnumerationResult res =
            enumerate_planar_unlabeled_graphs(n, true);
...
TEST(PlanarUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::PlanarLabeledEnumerationResult labeled =
            enumerate_planar_labeled_graphs_reverse_search(n);
        ...
        PlanarUnlabeledEnumerationResult unlabeled =
