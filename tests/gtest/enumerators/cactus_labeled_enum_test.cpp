...
using graph_recognition::CactusLabeledEnumerationResult;
...
const char* kDir = "cactus_labeled_enum";

class CactusLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CactusLabeledEnumTest, CountAndAllValid) {
    ...
    CactusLabeledEnumerationResult res = enumerate_cactus_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_cactus(g).is_cactus) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CactusLabeledEnumTest,
