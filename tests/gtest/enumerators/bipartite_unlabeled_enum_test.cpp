...
using graph_recognition::BipartiteUnlabeledEnumerationResult;
using graph_recognition::Graph;
using graph_recognition::check_bipartite;
using graph_recognition::enumerate_bipartite_labeled_graphs_reverse_search;
using graph_recognition::enumerate_bipartite_unlabeled_graphs;
...
const char* kDir = "bipartite_unlabeled_enum";
...
class BipartiteUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(BipartiteUnlabeledEnumTest, CountAndAllValid) {
    ...
    BipartiteUnlabeledEnumerationResult res = enumerate_bipartite_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_bipartite(g).is_bipartite)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, BipartiteUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce A005142 (connected bipartite graphs) with
// every graph connected, valid and pairwise non-isomorphic.
TEST(BipartiteUnlabeledEnumConnectedTest, MatchesA005142) {
    const int expected[] = {1, 1, 1, 3, 5, 17, 44, 182};
    for (int n = 1; n <= 8; ++n) {
        BipartiteUnlabeledEnumerationResult res =
            enumerate_bipartite_unlabeled_graphs(n, true);
...
// labeled enumerator produces. Capped at n = 6 (5177 labeled graphs x 720
// permutations); n = 7 would be 184024 x 5040.
TEST(BipartiteUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::BipartiteLabeledEnumerationResult labeled =
            enumerate_bipartite_labeled_graphs_reverse_search(n);
        ...
        BipartiteUnlabeledEnumerationResult unlabeled =
