...
using graph_recognition::ComparabilityUnlabeledEnumerationResult;
using graph_recognition::Graph;
using graph_recognition::check_comparability;
using graph_recognition::enumerate_comparability_labeled_graphs_reverse_search;
using graph_recognition::enumerate_comparability_unlabeled_graphs;
...
const char* kDir = "comparability_unlabeled_enum";
...
class ComparabilityUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ComparabilityUnlabeledEnumTest, CountAndAllValid) {
    ...
    ComparabilityUnlabeledEnumerationResult res = enumerate_comparability_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 6793 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
        ...
        EXPECT_TRUE(check_comparability(g).is_comparability)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ComparabilityUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected comparability graph counts
// (1, 1, 2, 6, 20, 101, 646, ...; not in the OEIS), with every graph
// connected, valid and pairwise non-isomorphic.
TEST(ComparabilityUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 6, 20, 101, 646};
    for (int n = 1; n <= 7; ++n) {
        ComparabilityUnlabeledEnumerationResult res =
            enumerate_comparability_unlabeled_graphs(n, true);
...
// labeled graph, and there are 30164 labeled comparability graphs at n = 6).
TEST(ComparabilityUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::ComparabilityLabeledEnumerationResult labeled =
            enumerate_comparability_labeled_graphs_reverse_search(n);
        ...
        ComparabilityUnlabeledEnumerationResult unlabeled =
