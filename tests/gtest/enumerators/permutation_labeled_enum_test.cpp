...
using graph_recognition::PermutationLabeledEnumerationResult;
...
const char* kDir = "permutation_labeled_enum";

class PermutationLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(PermutationLabeledEnumTest, CountAndAllValid) {
    ...
    PermutationLabeledEnumerationResult res = enumerate_permutation_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_permutation(g).is_permutation) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, PermutationLabeledEnumTest,
