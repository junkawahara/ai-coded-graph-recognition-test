...
using graph_recognition::ParityLabeledEnumerationResult;
using graph_recognition::check_parity;
using graph_recognition::enumerate_parity_labeled_graphs_reverse_search;
...
const char* kDir = "parity_labeled_enum";

class ParityLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ParityLabeledEnumTest, CountAndAllValid) {
    ...
    ParityLabeledEnumerationResult res = enumerate_parity_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_parity(g).is_parity) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ParityLabeledEnumTest,
