...
using graph_recognition::PtolemaicUnlabeledEnumerationResult;
using graph_recognition::check_ptolemaic;
using graph_recognition::enumerate_ptolemaic_labeled_graphs_reverse_search;
using graph_recognition::enumerate_ptolemaic_unlabeled_graphs;
...
const char* kDir = "ptolemaic_unlabeled_enum";
...
// 1053 graphs x 40320 permutations).
...
class PtolemaicUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(PtolemaicUnlabeledEnumTest, CountAndAllValid) {
    ...
    PtolemaicUnlabeledEnumerationResult res = enumerate_ptolemaic_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_ptolemaic(g).is_ptolemaic)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, PtolemaicUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce A287888 (connected non-isomorphic Ptolemaic
// graphs) with every graph connected, valid and pairwise non-isomorphic.
TEST(PtolemaicUnlabeledEnumConnectedTest, MatchesA287888) {
    const int expected[] = {1, 1, 2, 5, 14, 47, 170, 676};
    for (int n = 1; n <= 8; ++n) {
        PtolemaicUnlabeledEnumerationResult res =
            enumerate_ptolemaic_unlabeled_graphs(n, true);
...
// labeled enumerator produces. Capped at n = 6 (13534 labeled graphs x 720
// permutations); n = 7 would be 312125 x 5040.
TEST(PtolemaicUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::PtolemaicLabeledEnumerationResult labeled =
            enumerate_ptolemaic_labeled_graphs_reverse_search(n);
        ...
        PtolemaicUnlabeledEnumerationResult unlabeled =
