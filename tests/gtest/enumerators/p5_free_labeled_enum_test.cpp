...
using graph_recognition::P5FreeLabeledEnumerationResult;
using graph_recognition::check_p5_free;
using graph_recognition::enumerate_p5_free_labeled_graphs_reverse_search;
...
const char* kDir = "p5_free_labeled_enum";

class P5FreeLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(P5FreeLabeledEnumTest, CountAndAllValid) {
    ...
    P5FreeLabeledEnumerationResult res = enumerate_p5_free_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_p5_free(g).is_p5_free) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, P5FreeLabeledEnumTest,
