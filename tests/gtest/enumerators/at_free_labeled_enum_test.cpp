...
using graph_recognition::ATFreeLabeledEnumerationResult;
using graph_recognition::check_at_free;
using graph_recognition::enumerate_at_free_labeled_graphs_reverse_search;
...
const char* kDir = "at_free_labeled_enum";

class ATFreeLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ATFreeLabeledEnumTest, CountAndAllValid) {
    ...
    ATFreeLabeledEnumerationResult res = enumerate_at_free_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_at_free(g).is_at_free) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ATFreeLabeledEnumTest,
