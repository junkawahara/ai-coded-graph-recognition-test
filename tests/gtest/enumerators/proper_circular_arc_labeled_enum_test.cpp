...
using graph_recognition::ProperCircularArcLabeledEnumerationResult;
using graph_recognition::check_proper_circular_arc;
using graph_recognition::enumerate_proper_circular_arc_labeled_graphs_reverse_search;
...
const char* kDir = "proper_circular_arc_labeled_enum";

class ProperCircularArcLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ProperCircularArcLabeledEnumTest, CountAndAllValid) {
    ...
    ProperCircularArcLabeledEnumerationResult res = enumerate_proper_circular_arc_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_proper_circular_arc(g).is_proper_circular_arc) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ProperCircularArcLabeledEnumTest,
