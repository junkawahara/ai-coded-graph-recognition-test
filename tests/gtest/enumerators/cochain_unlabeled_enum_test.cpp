...
using graph_recognition::CochainUnlabeledEnumerationResult;
using graph_recognition::check_cochain;
using graph_recognition::enumerate_cochain_unlabeled_graphs;
...
const char* kDir = "cochain_unlabeled_enum";

class CochainUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CochainUnlabeledEnumTest, CountAndAllValid) {
    ...
    CochainUnlabeledEnumerationResult res = enumerate_cochain_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_cochain(g).is_cochain) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CochainUnlabeledEnumTest,
