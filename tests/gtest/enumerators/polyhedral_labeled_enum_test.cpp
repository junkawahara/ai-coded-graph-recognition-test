...
using graph_recognition::PolyhedralLabeledEnumerationResult;
...
const char* kDir = "polyhedral_labeled_enum";

class PolyhedralLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(PolyhedralLabeledEnumTest, CountAndAllValid) {
    ...
    PolyhedralLabeledEnumerationResult res = enumerate_polyhedral_labeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_polyhedral(g).is_polyhedral) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, PolyhedralLabeledEnumTest,
