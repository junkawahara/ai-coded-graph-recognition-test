...
using graph_recognition::ApexLabeledEnumerationResult;
using graph_recognition::check_apex;
using graph_recognition::enumerate_apex_labeled_graphs_reverse_search;
...
const char* kDir = "apex_labeled_enum";

class ApexLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ApexLabeledEnumTest, CountAndAllValid) {
    ...
    ApexLabeledEnumerationResult res = enumerate_apex_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_apex(g).is_apex) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ApexLabeledEnumTest,
