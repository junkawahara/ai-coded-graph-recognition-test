...
using graph_recognition::CoIntervalUnlabeledEnumerationResult;
using graph_recognition::check_co_interval;
using graph_recognition::enumerate_co_interval_labeled_graphs_reverse_search;
using graph_recognition::enumerate_co_interval_unlabeled_graphs;
...
const char* kDir = "co_interval_unlabeled_enum";

class CoIntervalUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CoIntervalUnlabeledEnumTest, CountAndAllValid) {
    ...
    CoIntervalUnlabeledEnumerationResult res = enumerate_co_interval_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_co_interval(g).is_co_interval) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CoIntervalUnlabeledEnumTest,
...
// labeled graph, and there are 17914 labeled co-interval graphs at n = 6).
TEST(CoIntervalUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::CoIntervalLabeledEnumerationResult labeled =
            enumerate_co_interval_labeled_graphs_reverse_search(n);
        ...
        CoIntervalUnlabeledEnumerationResult unlabeled =
