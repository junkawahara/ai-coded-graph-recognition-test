...
using graph_recognition::TriangleFreeUnlabeledEnumerationResult;
using graph_recognition::check_triangle_free;
using graph_recognition::enumerate_triangle_free_labeled_graphs_reverse_search;
using graph_recognition::enumerate_triangle_free_unlabeled_graphs;
...
const char* kDir = "triangle_free_unlabeled_enum";
...
class TriangleFreeUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(TriangleFreeUnlabeledEnumTest, CountAndAllValid) {
    ...
    TriangleFreeUnlabeledEnumerationResult res =
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_triangle_free(g).is_triangle_free)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, TriangleFreeUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce A024607 (connected triangle-free graphs)
// with every graph connected, valid and pairwise non-isomorphic.
TEST(TriangleFreeUnlabeledEnumConnectedTest, MatchesA024607) {
    const int expected[] = {1, 1, 1, 3, 6, 19, 59, 267};
    for (int n = 1; n <= 8; ++n) {
        TriangleFreeUnlabeledEnumerationResult res =
            enumerate_triangle_free_unlabeled_graphs(n, true);
...
// labeled enumerator produces. Capped at n = 6 (5789 labeled graphs x 720
// permutations); n = 7 would be 133501 x 5040.
TEST(TriangleFreeUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::TriangleFreeLabeledEnumerationResult labeled =
            enumerate_triangle_free_labeled_graphs_reverse_search(n);
        ...
        TriangleFreeUnlabeledEnumerationResult unlabeled =
            enumerate_triangle_free_unlabeled_graphs(n);
