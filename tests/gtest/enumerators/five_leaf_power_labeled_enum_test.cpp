...
using graph_recognition::FiveLeafPowerLabeledEnumerationResult;
using graph_recognition::check_five_leaf_power;
using graph_recognition::enumerate_five_leaf_power_labeled_graphs_reverse_search;
...
const char* kDir = "five_leaf_power_labeled_enum";

class FiveLeafPowerLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(FiveLeafPowerLabeledEnumTest, CountAndAllValid) {
    ...
    FiveLeafPowerLabeledEnumerationResult res = enumerate_five_leaf_power_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_five_leaf_power(g).is_five_leaf_power) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, FiveLeafPowerLabeledEnumTest,
