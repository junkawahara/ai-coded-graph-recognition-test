...
using graph_recognition::CaterpillarUnlabeledEnumerationResult;
using graph_recognition::check_caterpillar;
using graph_recognition::enumerate_caterpillar_unlabeled_graphs;
...
const char* kDir = "caterpillar_unlabeled_enum";

class CaterpillarUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CaterpillarUnlabeledEnumTest, CountAndAllValid) {
    ...
    CaterpillarUnlabeledEnumerationResult res = enumerate_caterpillar_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_caterpillar(g).is_caterpillar) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CaterpillarUnlabeledEnumTest,
