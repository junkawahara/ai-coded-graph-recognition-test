...
using graph_recognition::CircularArcLabeledEnumerationResult;
using graph_recognition::check_circular_arc;
using graph_recognition::enumerate_circular_arc_labeled_graphs_reverse_search;
...
const char* kDir = "circular_arc_labeled_enum";

class CircularArcLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(CircularArcLabeledEnumTest, CountAndAllValid) {
    ...
    CircularArcLabeledEnumerationResult res = enumerate_circular_arc_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_circular_arc(g).is_circular_arc) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, CircularArcLabeledEnumTest,
