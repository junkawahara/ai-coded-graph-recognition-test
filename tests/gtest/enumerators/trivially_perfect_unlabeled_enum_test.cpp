...
using graph_recognition::TriviallyPerfectUnlabeledEnumerationResult;
using graph_recognition::check_trivially_perfect;
using graph_recognition::enumerate_trivially_perfect_labeled_graphs_uvd;
using graph_recognition::enumerate_trivially_perfect_unlabeled_graphs;
...
const char* kDir = "trivially_perfect_unlabeled_enum";
...
class TriviallyPerfectUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(TriviallyPerfectUnlabeledEnumTest, CountAndAllValid) {
    ...
    TriviallyPerfectUnlabeledEnumerationResult res =
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_trivially_perfect(g).is_trivially_perfect)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, TriviallyPerfectUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce A000081 (rooted trees on n nodes) with every
// graph connected, valid and pairwise non-isomorphic.
TEST(TriviallyPerfectUnlabeledEnumConnectedTest, MatchesA000081) {
    const int expected[] = {1, 1, 2, 4, 9, 20, 48, 115};
    for (int n = 1; n <= 8; ++n) {
        TriviallyPerfectUnlabeledEnumerationResult res =
            enumerate_trivially_perfect_unlabeled_graphs(n, true);
...
// labeled enumerator produces. Capped at n = 6 (4144 labeled graphs x 720
// permutations); n = 7 would be 51515 x 5040.
TEST(TriviallyPerfectUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::TriviallyPerfectLabeledEnumerationResult labeled =
            enumerate_trivially_perfect_labeled_graphs_uvd(n);
        ...
        TriviallyPerfectUnlabeledEnumerationResult unlabeled =
            enumerate_trivially_perfect_unlabeled_graphs(n);
