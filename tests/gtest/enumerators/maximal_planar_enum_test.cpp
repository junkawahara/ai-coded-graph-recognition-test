...
using graph_recognition::MaximalPlanarEnumerationResult;
using graph_recognition::check_maximal_planar;
using graph_recognition::enumerate_maximal_planar_graphs_reverse_search;
...
const char* kDir = "maximal_planar_enum";

class MaximalPlanarEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(MaximalPlanarEnumTest, CountAndAllValid) {
    ...
    MaximalPlanarEnumerationResult res = enumerate_maximal_planar_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_maximal_planar(g).is_maximal_planar) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, MaximalPlanarEnumTest,
