...
using graph_recognition::LamanLabeledEnumerationResult;
...
const char* kDir = "laman_labeled_enum";

class LamanLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(LamanLabeledEnumTest, CountAndAllValid) {
    ...
    LamanLabeledEnumerationResult res = enumerate_laman_labeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_laman(g).is_laman) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, LamanLabeledEnumTest,
