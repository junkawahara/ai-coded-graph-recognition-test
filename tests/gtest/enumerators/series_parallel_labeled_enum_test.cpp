...
using graph_recognition::SeriesParallelLabeledEnumerationResult;
...
const char* kDir = "series_parallel_labeled_enum";

class SeriesParallelLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(SeriesParallelLabeledEnumTest, CountAndAllValid) {
    ...
    SeriesParallelLabeledEnumerationResult res = enumerate_series_parallel_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_series_parallel(g).is_series_parallel) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, SeriesParallelLabeledEnumTest,
