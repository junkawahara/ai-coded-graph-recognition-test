...
using graph_recognition::DiamondFreeLabeledEnumerationResult;
using graph_recognition::check_diamond_free;
using graph_recognition::enumerate_diamond_free_labeled_graphs_reverse_search;
...
const char* kDir = "diamond_free_labeled_enum";

class DiamondFreeLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(DiamondFreeLabeledEnumTest, CountAndAllValid) {
    ...
    DiamondFreeLabeledEnumerationResult res = enumerate_diamond_free_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_diamond_free(g).is_diamond_free) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, DiamondFreeLabeledEnumTest,
