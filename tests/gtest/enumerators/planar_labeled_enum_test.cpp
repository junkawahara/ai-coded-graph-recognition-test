...
using graph_recognition::PlanarLabeledEnumerationResult;
...
const char* kDir = "planar_labeled_enum";

class PlanarLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(PlanarLabeledEnumTest, CountAndAllValid) {
    ...
    PlanarLabeledEnumerationResult res = enumerate_planar_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_planar(g).is_planar) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, PlanarLabeledEnumTest,
