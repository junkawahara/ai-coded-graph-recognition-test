...
using graph_recognition::SeriesParallelUnlabeledEnumerationResult;
using graph_recognition::check_series_parallel;
using graph_recognition::enumerate_series_parallel_labeled_graphs_reverse_search;
...
const char* kDir = "series_parallel_unlabeled_enum";
...
class SeriesParallelUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(SeriesParallelUnlabeledEnumTest, CountAndAllValid) {
    ...
    SeriesParallelUnlabeledEnumerationResult res = enumerate_series_parallel_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        // permutation; guarded to n <= 7, since n = 8 would be 1715 graphs
        // times 8! orderings).
        if (res.graphs[i].n <= 7) {
        ...
        EXPECT_TRUE(check_series_parallel(g).is_series_parallel)
            << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, SeriesParallelUnlabeledEnumTest,
    ::testing::ValuesIn(list_in_files(test_path(kDir))),
    [](const ::testing::TestParamInfo<std::string>& info) { return info.param; });

// connected_only must reproduce the connected series-parallel graph counts
// (verified independently against a direct K4-minor filter; not in the OEIS),
// with every graph connected, valid and pairwise non-isomorphic.
TEST(SeriesParallelUnlabeledEnumConnectedTest, MatchesConnectedCounts) {
    const int expected[] = {1, 1, 2, 5, 15, 56, 241};
    for (int n = 1; n <= 7; ++n) {
        SeriesParallelUnlabeledEnumerationResult res =
...
TEST(SeriesParallelUnlabeledEnumCrossCheckTest, MatchesLabeledEnumeratorClasses) {
    for (int n = 1; n <= 6; ++n) {
        std::set<std::vector<std::pair<int, int> > > labeled_classes;
        graph_recognition::SeriesParallelLabeledEnumerationResult labeled =
            enumerate_series_parallel_labeled_graphs_reverse_search(n);
        ...
        SeriesParallelUnlabeledEnumerationResult unlabeled =
