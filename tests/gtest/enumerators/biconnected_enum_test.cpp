...
using graph_recognition::BiconnectedEnumerationResult;
using graph_recognition::check_biconnected;
using graph_recognition::enumerate_biconnected_graphs;
...
const char* kDir = "biconnected_enum";

class BiconnectedEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(BiconnectedEnumTest, CountAndAllValid) {
    ...
    BiconnectedEnumerationResult res = enumerate_biconnected_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_biconnected(g).is_biconnected) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, BiconnectedEnumTest,
