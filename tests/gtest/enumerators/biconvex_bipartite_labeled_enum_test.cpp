...
using graph_recognition::BiconvexBipartiteLabeledEnumerationResult;
using graph_recognition::check_biconvex_bipartite;
using graph_recognition::enumerate_biconvex_bipartite_labeled_graphs_reverse_search;
...
const char* kDir = "biconvex_bipartite_labeled_enum";

class BiconvexBipartiteLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(BiconvexBipartiteLabeledEnumTest, CountAndAllValid) {
    ...
    BiconvexBipartiteLabeledEnumerationResult res = enumerate_biconvex_bipartite_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_biconvex_bipartite(g).is_biconvex_bipartite) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, BiconvexBipartiteLabeledEnumTest,
