...
using graph_recognition::ForestUnlabeledEnumerationResult;
using graph_recognition::check_forest;
...
const char* kDir = "forest_unlabeled_enum";

class ForestUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ForestUnlabeledEnumTest, CountAndAllValid) {
    ...
    ForestUnlabeledEnumerationResult res = enumerate_forest_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_forest(g).is_forest) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ForestUnlabeledEnumTest,
