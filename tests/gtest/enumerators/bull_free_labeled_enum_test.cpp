...
using graph_recognition::BullFreeLabeledEnumerationResult;
using graph_recognition::check_bull_free;
using graph_recognition::enumerate_bull_free_labeled_graphs_reverse_search;
...
const char* kDir = "bull_free_labeled_enum";

class BullFreeLabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(BullFreeLabeledEnumTest, CountAndAllValid) {
    ...
    BullFreeLabeledEnumerationResult res = enumerate_bull_free_labeled_graphs_reverse_search(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_bull_free(g).is_bull_free) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, BullFreeLabeledEnumTest,
