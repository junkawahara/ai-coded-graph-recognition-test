...
using graph_recognition::ChainUnlabeledEnumerationResult;
using graph_recognition::check_chain;
using graph_recognition::enumerate_chain_unlabeled_graphs;
...
const char* kDir = "chain_unlabeled_enum";

class ChainUnlabeledEnumTest : public ::testing::TestWithParam<std::string> {};

TEST_P(ChainUnlabeledEnumTest, CountAndAllValid) {
    ...
    ChainUnlabeledEnumerationResult res = enumerate_chain_unlabeled_graphs(n);
    ...
    for (size_t i = 0; i < res.graphs.size(); ++i) {
        ...
        EXPECT_TRUE(check_chain(g).is_chain) << "invalid graph in case=" << stem;
    }
}

INSTANTIATE_TEST_SUITE_P(
    AllCases, ChainUnlabeledEnumTest,
