...
def is_threshold(n, edges):
    """Check if graph is threshold by iterative removal of isolated/dominating vertices."""
    ...
    for _ in range(n):
        ...
        for v in range(1, n + 1):
            if not alive[v]:
                continue
            deg = sum(1 for u in adj[v] if alive[u])
            if deg == 0 or deg == alive_count - 1:
                alive[v] = False
                alive_count -= 1
                found = True
                break
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_threshold_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_threshold(n, edges):
            print("non-threshold graph found:", edges)
            return 1
