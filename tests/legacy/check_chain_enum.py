...
def is_chain(n, edges):
    """Check if graph is a chain graph (bipartite with nested neighborhoods)."""
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_chain_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_chain(n, edges):
            print("non-chain graph found:", edges)
            return 1
