    ...
    for u, v in edges:
        ...
        for i in range(len(common_list)):
            for j in range(i + 1, len(common_list)):
                if common_list[j] not in adj[common_list[i]]:
                    return False
    return True


def is_block(n, edges):
    return is_chordal(n, edges) and is_diamond_free(n, edges)


def main():
    if len(sys.argv) != 4:
        print("usage: check_block_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_block(n, edges):
            print("non-block graph found:", edges)
            return 1
