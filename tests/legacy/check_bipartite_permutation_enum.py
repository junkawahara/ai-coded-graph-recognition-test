...
def is_permutation(n, edges):
    """Check if graph is a permutation graph (= comparability AND co-comparability)."""
    edge_set = set()
    for u, v in edges:
        edge_set.add((u, v))
        edge_set.add((v, u))

    # Check comparability: G has a transitive orientation
    if not has_transitive_orientation(n, edge_set):
        return False

    # Check co-comparability: complement has a transitive orientation
    comp_edge_set = set()
    for u in range(1, n + 1):
        for v in range(u + 1, n + 1):
            if (u, v) not in edge_set:
                comp_edge_set.add((u, v))
                comp_edge_set.add((v, u))
    if not has_transitive_orientation(n, comp_edge_set):
        return False
...
def has_transitive_orientation(n, edge_set):
    ...
    # Try all 2^m orientations (only feasible for small m)
    if m > 20:
...
def is_bipartite_permutation(n, edges):
    return is_bipartite(n, edges) and is_permutation(n, edges)


def main():
    if len(sys.argv) != 4:
        print("usage: check_bipartite_permutation_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_bipartite_permutation(n, edges):
            print("non-bipartite-permutation graph found:", edges)
            return 1
