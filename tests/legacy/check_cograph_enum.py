...
def is_cograph(n, edges):
    """Check P4-free by searching for induced P4."""
    adj = [set() for _ in range(n + 1)]
    for u, v in edges:
        adj[u].add(v)
        adj[v].add(u)

    # Check all 4-tuples for induced P4: a-b-c-d
    for b in range(1, n + 1):
        for c in adj[b]:
            if c <= b:
                continue
            # b-c edge exists; look for a adjacent to b but not c,
            # and d adjacent to c but not a, not b
            for a in adj[b]:
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_cograph_enum.py <in> <exp> <out>")
        return 2
