...
def is_bull_free(n, edges):
    """Check if graph is bull-free."""
    adj = [set() for _ in range(n + 1)]
    for u, v in edges:
        adj[u].add(v)
        adj[v].add(u)
    # bull: triangle {a,b,c} + pendant a-x + pendant b-y
    # edges: ab, ac, bc, ax, by
    # non-edges: bx, cx, ay, cy, xy
    for subset in combinations(range(1, n + 1), 5):
        for a, b, c, x, y in permutations(subset):
            if (b in adj[a] and c in adj[a] and c in adj[b] and
                x in adj[a] and y in adj[b] and
                x not in adj[b] and x not in adj[c] and
                y not in adj[a] and y not in adj[c] and
                y not in adj[x]):
                return False
    return True
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_bull_free_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_bull_free(n, edges):
            print("non-bull-free graph found:", edges)
            return 1
