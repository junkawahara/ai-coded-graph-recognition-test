...
def is_odd_hole_free(n, edges):
    """Check if graph is odd-hole-free (no induced odd cycle of length >= 5)."""
    adj = [set() for _ in range(n + 1)]
    for u, v in edges:
        adj[u].add(v)
        adj[v].add(u)
    # Check all subsets of size >= 5 (odd sizes) for induced cycles
    for size in range(5, n + 1, 2):
        for subset in combinations(range(1, n + 1), size):
            s = set(subset)
            # Check if subset induces a cycle: each vertex has exactly 2 neighbors in s
            is_cycle = True
            for v in subset:
                nbrs_in_s = sum(1 for w in subset if w in adj[v])
                if nbrs_in_s != 2:
                    is_cycle = False
                    break
            if not is_cycle:
                continue
            # Verify connectivity (a cycle on k vertices with each degree 2 is connected)
            # Actually, degree-2 regular on k vertices could be disjoint cycles,
            # but only if k >= 6. For k=5, degree-2 regular must be C5.
            # For general k, check connectivity.
            visited = set()
            stack = [subset[0]]
            while stack:
                cur = stack.pop()
                ...
                visited.add(cur)
                for w in subset:
                    if w in adj[cur] and w not in visited:
                        stack.append(w)
            if len(visited) == size:
                return False
    return True
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_odd_hole_free_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_odd_hole_free(n, edges):
            print("non-odd-hole-free graph found:", edges)
            return 1
