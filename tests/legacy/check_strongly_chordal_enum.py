...
def is_strongly_chordal(n, edges):
    """Simple vertex elimination で強弦グラフか判定する。"""
    ...
    for _ in range(n):
        ...
        for v in sorted(alive):
            nbrs = [u for u in alive if u != v and u in adj[v]]

            # simplicial チェック: 近傍がクリーク
            simplicial = True
            ...
            # simple チェック: 近傍の閉近傍が包含で全順序
            def closed_nbhd(u):
                return frozenset({u} | (adj[u] & alive))

            simple = True
            for a in range(len(nbrs)):
                for b in range(a + 1, len(nbrs)):
                    na = closed_nbhd(nbrs[a])
                    nb = closed_nbhd(nbrs[b])
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_strongly_chordal_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_strongly_chordal(n, edges):
            print("non-strongly-chordal graph found:", edges)
            return 1
