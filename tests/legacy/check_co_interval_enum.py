...
def has_asteroidal_triple(n, edges):
    """Brute-force AT detection via BFS."""
    if n < 3:
        return False
    adj = [set() for _ in range(n + 1)]
    for u, v in edges:
    ...
    def reachable_avoiding(s, closed_nbr):
        """BFS from s avoiding all vertices in closed_nbr."""
        if s in closed_nbr:
            return set()
        visited = {s}
        q = deque([s])
        while q:
            cur = q.popleft()
            for nb in adj[cur]:
                if nb not in visited and nb not in closed_nbr:
                    visited.add(nb)
                    q.append(nb)
        return visited
    ...
    for i in range(len(vertices)):
        ...
        for j in range(i + 1, len(vertices)):
            ...
            for k in range(j + 1, len(vertices)):
                c = vertices[k]
                closed_c = {c} | adj[c]
                # Check: path b-c avoiding N[a]
                reach_a = reachable_avoiding(b, closed_a)
                if c not in reach_a:
                    continue
                # Check: path a-c avoiding N[b]
                reach_b = reachable_avoiding(a, closed_b)
                if c not in reach_b:
                    continue
                # Check: path a-b avoiding N[c]
                reach_c = reachable_avoiding(a, closed_c)
                if b not in reach_c:
                    continue
                return True
    return False
...
def is_interval(n, edges):
    """Interval = chordal + AT-free."""
    return is_chordal(n, edges) and not has_asteroidal_triple(n, edges)


def is_co_interval(n, edges):
    """Check if graph is co-interval (complement is interval)."""
    edge_set = set()
    for u, v in edges:
    ...
    return is_interval(n, complement_edges)


def main():
    if len(sys.argv) != 4:
        print("usage: check_co_interval_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_co_interval(n, edges):
            print("non-co-interval graph found:", edges)
            return 1
