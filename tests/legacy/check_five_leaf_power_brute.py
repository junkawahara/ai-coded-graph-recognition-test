#!/usr/bin/env python3
"""
5-leaf power brute force comparison checker.
Validates C++ recognition against random graphs and constructive 5-leaf powers.
"""
import itertools
import random
import subprocess
...
def is_five_leaf_power_brute(n, edges_1indexed):
    """Python brute force 5-leaf power check"""
    if n <= 1:
        return True
...
def generate_random_5lp(n):
    """Construct random 5-leaf power from a random Steiner tree"""
    if n <= 1:
        return []
    ...
    # Random Prufer sequence to generate tree
    if total == 2:
        tree_edges = [(0, 1)]
    else:
    ...
    # Assign each vertex to a Steiner node
    assignment = [random.randint(n, n + s - 1) for _ in range(n)]

    # Compute distances
    dist = tree_dist(tree_edges, total)
...
def main():
    ...
    for t in range(num_tests):
        ...
        if random.random() < 0.5:
            ...
            expected = is_five_leaf_power_brute(n, edges)
