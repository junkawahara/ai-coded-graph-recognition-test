...
def is_proper_interval(n, edges):
    """Roberts characterization: proper interval iff there exists a vertex
    ordering where every closed neighborhood is a consecutive interval."""
    if n == 0:
        return True
    ...
    for perm in permutations(range(1, n + 1)):
        ...
        for v in range(1, n + 1):
            nbrs = sorted([pos[u] for u in adj[v]] + [pos[v]])
            for idx in range(1, len(nbrs)):
                if nbrs[idx] != nbrs[idx - 1] + 1:
                    ok = False
                    break
            if not ok:
                break
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_proper_interval_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_proper_interval(n, edges):
            print("non-proper-interval graph found:", edges)
            return 1
