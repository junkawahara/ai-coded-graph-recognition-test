...
def is_parity(n, edges):
    """Check if graph is a parity graph."""
    adj = [set() for _ in range(n + 1)]
    for u, v in edges:
        adj[u].add(v)
        adj[v].add(u)

    # BFS distances from each vertex
    for s in range(1, n + 1):
        ...
        for v in range(s + 1, n + 1):
            if d[v] < 0:
                continue
            target = d[v] % 2
            # Backtracking: find induced s-v path with different parity
            blocked = [0] * (n + 1)
            in_path = [False] * (n + 1)
            in_path[s] = True
            for w in adj[s]:
            ...
            def dfs(cur, depth):
                ...
                for w in adj[cur]:
                    ...
                    if in_path[w]:
                        continue
                    if w == v:
                        if blocked[v] == 1 and (depth + 1) % 2 != target:
                            found[0] = True
                            return
                        continue
                    if blocked[w] != 1:
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_parity_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_parity(n, edges):
            print("non-parity graph found:", edges)
            return 1
