...
def is_diamond_free(n, edges):
    """Check if graph is diamond-free (K4-e free)."""
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_diamond_free_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_diamond_free(n, edges):
            print("non-diamond-free graph found:", edges)
            return 1
