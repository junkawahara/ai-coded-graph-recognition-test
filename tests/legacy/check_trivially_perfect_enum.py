...
def is_trivially_perfect(n, edges):
    """Check {P4, C4}-free."""
    ...
    # Check for induced P4: a-b-c-d
    for b in range(1, n + 1):
        for c in adj[b]:
            if c <= b:
                continue
            for a in adj[b]:
                if a == c or a in adj[c]:
                    continue
                for d in adj[c]:
                    if d == b or d == a:
                        continue
                    if d in adj[b] or d in adj[a]:
                        continue
                    return False  # induced P4: a-b-c-d

    # Check for induced C4: a-b-c-d-a
        ...
        for b in adj[a]:
            ...
            for c in adj[b]:
                if c <= a or c == a:
                    continue
                if c in adj[a]:
                    continue  # a-c edge exists
                for d in adj[c]:
                    if d <= a or d == b:
                        continue
                    if d not in adj[a]:
                        continue  # need a-d edge for C4
                    if d in adj[b]:
                        continue  # b-d edge exists
                    return False  # induced C4: a-b-c-d-a
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_trivially_perfect_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_trivially_perfect(n, edges):
            print("non-trivially-perfect found:", edges)
            return 1
