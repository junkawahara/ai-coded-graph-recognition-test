...
def is_digraph(n, arcs):
    """Check if the arc set forms a valid simple digraph on {1, ..., n}."""
    arc_set = set(arcs)
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_digraph_enum.py <in> <exp> <out>")
        return 2
    ...
    for arcs in graphs:
        if not is_digraph(n, arcs):
            print("invalid digraph found:", arcs)
            return 1
