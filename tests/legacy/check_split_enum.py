...
def is_split(n, edges):
    """Hammer-Simeone degree sequence condition."""
    if n == 0:
        return True
    ...
    d = sorted([deg[v] for v in range(1, n + 1)], reverse=True)
    ...
    left_sum = sum(d[:m_val])
    right_sum = sum(d[m_val:])
    return left_sum == m_val * (m_val - 1) + right_sum


def main():
    if len(sys.argv) != 4:
        print("usage: check_split_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_split(n, edges):
            print("non-split graph found:", edges)
            return 1
