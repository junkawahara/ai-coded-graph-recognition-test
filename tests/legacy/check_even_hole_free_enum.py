...
def is_even_hole_free(n, edges):
    """Check if graph is even-hole-free (no induced even cycle of length >= 4)."""
    adj = [set() for _ in range(n + 1)]
    for u, v in edges:
        adj[u].add(v)
        adj[v].add(u)
    # Check all subsets of even size >= 4 for induced cycles
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_even_hole_free_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_even_hole_free(n, edges):
            print("non-even-hole-free graph found:", edges)
            return 1
