        ...
        for (int gi = 0; gi < co_comp_by_enum; ++gi) {
            ...
            if (!res.is_co_comparability) {
                std::cerr << "INVALID n=" << n << " graph#" << gi
                          << " is not co-comparability" << std::endl;
                mismatch++;
            }
        }

        // Compare counts
        if (co_comp_by_filter != co_comp_by_enum) {
            std::cerr << "MISMATCH n=" << n
                      << " filter=" << co_comp_by_filter
                      << " enum=" << co_comp_by_enum << std::endl;
            mismatch++;
        }

        std::cout << "n=" << n
                  << " total=" << (1LL << num_edges)
                  << " co_comparability=" << co_comp_by_enum << std::endl;
    }
