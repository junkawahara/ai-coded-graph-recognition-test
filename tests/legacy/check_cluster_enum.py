...
def is_cluster(n, edges):
    """Check if graph is a cluster graph (P3-free / disjoint union of cliques)."""
        ...
        while queue:
            ...
            for v in adj[u]:
                if not visited[v]:
                    visited[v] = True
                    queue.append(v)
        # Check component is a clique
        k = len(comp)
        for v in comp:
            if len(adj[v] & set(comp)) != k - 1:
                return False
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_cluster_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_cluster(n, edges):
            print("non-cluster graph found:", edges)
            return 1
