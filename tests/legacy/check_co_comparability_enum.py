...
def is_co_comparability(n, edges):
    """Check if graph is a co-comparability graph (complement admits a transitive orientation)."""
    edge_set = set()
    for u, v in edges:
    ...
    return has_transitive_orientation(n, complement_edges)


def main():
    if len(sys.argv) != 4:
        print("usage: check_co_comparability_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_co_comparability(n, edges):
            print("non-co-comparability graph found:", edges)
            return 1
