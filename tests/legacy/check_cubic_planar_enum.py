#!/usr/bin/env python3
"""Checker for cubic planar graph enumeration.

Validates that every output graph is cubic (3-regular) and planar.
Planarity is checked via edge count bound (m <= 3n-6) and
K5 / K3,3 subgraph detection (consistent with check_planar_enum.py).
"""
import sys
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_cubic_planar_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_cubic(n, edges):
            print("non-cubic graph found:", edges)
            return 1
        if not is_planar(n, edges):
            print("non-planar graph found:", edges)
            return 1
