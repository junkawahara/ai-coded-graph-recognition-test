...
def is_co_chordal(n, edges):
    """Check if graph is co-chordal (complement is chordal)."""
    edge_set = set()
    for u, v in edges:
    ...
    return is_chordal(n, complement_edges)


def main():
    if len(sys.argv) != 4:
        print("usage: check_co_chordal_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_co_chordal(n, edges):
            print("non-co-chordal graph found:", edges)
            return 1
