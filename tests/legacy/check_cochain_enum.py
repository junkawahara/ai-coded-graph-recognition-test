...
def is_cochain(n, edges):
    """Check if graph is a cochain graph (complement is a chain graph)."""
    # Build complement
    edge_set = set(edges)
    comp_edges = []
    ...
    # Check complement is chain (bipartite with nested neighborhoods)
        ...
        while queue:
            ...
            for v in adj[u]:
                if color[v] == -1:
                    ...
                    return False  # odd cycle in complement

    # Collect sides
    left = [v for v in range(1, n + 1) if color[v] == 0]
    right = [v for v in range(1, n + 1) if color[v] == 1]

    # Check nested neighborhoods on the left side
    for i in range(len(left)):
        for j in range(i + 1, len(left)):
            ni = adj[left[i]] & set(right)
            nj = adj[left[j]] & set(right)
            if not (ni <= nj or nj <= ni):
                return False
...
def main():
    if len(sys.argv) != 4:
        print("usage: check_cochain_enum.py <in> <exp> <out>")
        return 2
    ...
    for edges in graphs:
        if not is_cochain(n, edges):
            print("non-cochain graph found:", edges)
            return 1
